...
✅ **Про `/media/` — проверено (сессия 3Б).** Раздача через Django заводится
в `config/urls.py` **внутри `if settings.DEBUG`**, то есть на проде маршрута
нет вовсе. Строка в таблице выше описывает режим разработки. Подробности и
побочный вывод (загруженные файлы на проде не отдаются ничем) —
в [SECURITY.md](SECURITY.md#пользовательские-файлы).
...
|---|---|---|---|
| 1 | `calc2:export_pdf` | ввод клиента → внешний процесс `pdflatex` | ✅ **закрыто** (3Б): `openin_any=p`, `openout_any=p`, `shell_escape=f` — ограничение самого TeX, а не список запрещённых команд |
...
| 9 | `calendar_stub:event_update` | загрузка до проверки: 403 подтверждает существование | ✅ **закрыто** (3Б): `_own_event` сужает выборку, ответ 404 и на чужое, и на несуществующее. Заодно `event_delete` и `event_detail` |
| 10 | `/media/<path>` | путь от клиента; проверить, что при `DEBUG=False` маршрута нет | ✅ **проверено** (3Б): маршрут заводится внутри `if settings.DEBUG`, на проде его нет. Тест `test_media_route.py` разбирает `urls.py` синтаксисом, а не поиском строки |
...
Все три хвоста сессии 3А закрыты в сессии 3Б.

- ✅ **`event_update` грузил объект до проверки.** Сужено до `_own_event`;
  ответ 404 одинаков на чужое и на несуществующее. Фронтенд правки не
  потребовал: скрипт календаря разбирает ТЕЛО ответа (`d.status`), а не код,
  и ответ остался JSON. Заодно у удаления появился пропавший `.catch` —
  раньше любой не-JSON ронял промис молча.
- ✅ **`/media/<path>` при `DEBUG=False`** — маршрута нет: он заводится внутри
  `if settings.DEBUG`. WhiteNoise отдаёт только `STATIC_ROOT`
  (`WHITENOISE_ROOT` не задан), представлений с `FileResponse` в проекте нет.
  ⚠️ Побочный вывод: четыре поля с файлами на проде НЕ отдаются вовсе —
  недоделка, карточка в Notion.
- ✅ **Показ уже сохранённого.** Решение владельца: показывать с пометкой
  «снята с публикации» и без ссылки на публичную страницу. Молча спрятать
  нельзя (подборка «худеет» без объяснения), молча показать со ссылкой —
  тоже (тогда шлюз не работает).

### Что нашлось в сессии 3Б сверх этого списка

| Место | Что было | Серьёзность |
|---|---|---|
| `catalog:problem_list`, окно предпросмотра | Условие задачи из JSON склеивалось в строку разметки и уезжало в `innerHTML`. Публично, вход не нужен, корпус — 31 690 задач из внешних источников | **высокая** |
| `student/_problem_card.html` и три соседних шаблона | `\|safe` у условия и подпунктов — 8 мест. Тот же текст в каталоге всегда выводился экранированным | **высокая** |
| `calendar_stub`, `catalog`, `views_stats`, `game` | `json.dumps` внутри `<script>`: `<` не экранируется, `</script>` в названии занятия закрывал тег | средняя |
| `templates/404.html` | Наследовала `teacher/base.html`: звала в панель репетитора всех подряд и падала с `NoReverseMatch` на наборе адресов без пространства `catalog` — то есть 404 превращалась в 500 | низкая |
| Приёмник отчётов CSP (заведён в этой же сессии) | Публичный адрес, пишущий в журнал: без ограничения тела и без отсева пустых отчётов он сам был бы способом забить журнал | низкая (закрыто сразу) |
