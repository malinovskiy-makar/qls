...
⚠️ **`teacher_required` не знает про `UserProfile`.** `tutor_required` смотрит
обе системы ролей, `teacher_required` — только старое поле `User.role`.
Репетитор, заведённый через профиль (`profile.role == 'tutor'`) и **без**
старого `User.role == 'teacher'`, пройдёт на 95 маршрутов кабинета и получит
403 на шести. Список этих шести — в таблице ниже, помечены `teacher_required`.

Сейчас это не стреляет, потому что роли синхронизируются при заведении
пользователя. Но это **два источника истины об одном и том же**, и держатся
они на том, что синхронизация нигде не рассыпалась.
...
| # | Место | Почему в списке |
|---|---|---|
| 1 | `calc2:export_pdf` | ввод клиента → внешний процесс `pdflatex` |
| 2 | `teacher:assignment_print` | единственный маршрут кабинета без декоратора; гость даёт не 403, а ошибку |
| 3 | `calendar_stub:event_delete` | массовое удаление серии; право проверено на одном звене |
| 4 | Подборки по токену (7 маршрутов) | знание строки = право на запись, отзыва нет |
| 5 | `teacher_required` против `tutor_required` | два источника истины о роли, шесть маршрутов на старом |
| 6 | `teacher:api_comment_create` | единственная граница без декоратора |
| 7 | `teacher:student_work_review_plain` | тот же обработчик без `group_id` |
| 8 | `/parent/*` | роль не проверяется, границу держит форма запроса |
| 9 | `calendar_stub:event_update` | загрузка до проверки: 403 подтверждает существование |
| 10 | `/media/<path>` | путь от клиента; проверить, что при `DEBUG=False` маршрута нет |
| 11 | Очки Econ Rush | состояние в сессии браузера, доверенными не считать |
| 12 | `cart_print` | `@tutor_required` дважды — след правки вслепую |
