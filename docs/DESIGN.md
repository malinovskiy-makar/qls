...
> ⚠️ **Бренд — Weconomics** (решение владельца 2026-08-19). Имя «ЭкЗадачи»
> ниже по тексту — прежнее; переименование интерфейса, писем и разметки —
> отдельная работа, до неё документ описывает то, что сейчас на экране.
