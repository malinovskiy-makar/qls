...
отключён 2026-08 (истёк пробный период). Что раньше обеспечивал Render и что
нужно настроить и проверить на новом хостинге —
[docs/MIGRATION-CHECKLIST.md](MIGRATION-CHECKLIST.md).

Команда деплоя (`git push origin main` → автовыкатка) и способ отката зависят
от устройства конкретного хостинга — писать раньше выбора рано.

**Проверка после выката (не зависит от хостинга):** открыть главную,
`/catalog/`, страницу любой задачи.
