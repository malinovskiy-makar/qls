...
### Заливка ЧАСТИ корпуса, а не всего банка

С 12.09.2026 `dump_for_deploy` умеет ограничиваться явным списком задач.
Нужно это потому, что на прод едет строго отобранное подмножество, а не весь
банк ([ADR 0098](adr/0098-corpus-transfer-strict-whitelist.md)).

```bash
manage.py dump_for_deploy --bank-only     --ids-file reports/corpus_transfer_20260912/transfer_new_ids.txt     --with-dupmark     --dupmark-scope-file reports/corpus_transfer_20260912/transfer_scope_ids.txt     --outdir deploy_fixtures_transfer --chunk 2000
```

| флаг | что делает |
|---|---|
| `--ids-file` | выгрузить только эти задачи и всё, что на них ссылается |
| `--with-dupmark` | добавить пометки групп копий, только целыми группами |
| `--dupmark-scope-file` | что будет на проде ПОСЛЕ заливки |

⚠️ **В список кладут ТОЛЬКО новые задачи, а не «нынешние плюс новые».**
Заливка пропускает задачу с занятым `pk`, но её подпункты, ссылки и картинки
имеют СВОИ номера, и они не пропустятся: на проде появился бы второй
экземпляр каждого подпункта. Поэтому область видимости групп задаётся
отдельным файлом — группа копий может стоять одной ногой в уже залитой
задаче, которую повторно выгружать нельзя.

⚠️ **Модель, для которой нет пути до задачи в `PROBLEM_PATH`, роняет
команду.** Это намеренно: выгрузить её целиком молча значило бы увезти на
прод строку, ссылающуюся на задачу, которой там нет.
