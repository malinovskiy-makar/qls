...
| Том media (с 24.09.2026) | `/srv/weconomics/backups/weconomics-media-ГГГГММДД-ЧЧММСС.tar.gz` — tar.gz тома `weconomics_media`, контейнер `alpine` монтирует том только на чтение, затем `gzip -t` |
| Права | каталог 700, файлы 600, владелец root |
| Хранить | 30 копий каждого вида, лишние удаляются по имени (в нём дата) |
| Внешняя копия | `BACKUP_S3_*` в `.env`, **пока пусто**; когда заполнят — уходят и дамп, и архив media |

⚠️ **До 24.09.2026 том `media` не бэкапился вовсе.** В нём аватары,
вложения чата и фото решений учеников; в базе только пути к ним, и дамп
без тома восстанавливал бы ссылки в никуда.
...
Архив тома — обычный `tar.gz` с корнем тома внутри (`./avatars/…`,
`./chat/…` и т. д.). Проверять так же, как дамп: сначала во временный том,
а не поверх боевого.

```bash
LAST_MEDIA=$(sudo sh -c 'ls -1t /srv/weconomics/backups/weconomics-media-*.tar.gz | head -1')

# 1. Что внутри — без распаковки.
sudo tar -tzf "$LAST_MEDIA" | head -20
sudo tar -tzf "$LAST_MEDIA" | wc -l

# 2. Проверочное восстановление во ВРЕМЕННЫЙ том и сверка числа файлов.
docker volume create weconomics_media_restore_test
sudo cat "$LAST_MEDIA" | docker run --rm -i   -v weconomics_media_restore_test:/m alpine:3.20 tar -xzf - -C /m
docker run --rm -v weconomics_media_restore_test:/m:ro alpine:3.20   sh -c 'find /m -type f | wc -l'
docker run --rm -v weconomics_media:/m:ro alpine:3.20   sh -c 'find /m -type f | wc -l'
docker volume rm weconomics_media_restore_test
```

⚠️ **Настоящая авария — поверх боевого тома, и только осознанно.**
Остановить `web` (`docker compose stop web`), распаковать архив в
`weconomics_media` той же командой, что в шаге 2, но с боевым томом,
поднять `web`. Файлы, которых в архиве нет, распаковка не удаляет — это
безопаснее, чем очистка тома перед ней. Дамп базы и архив media стоит брать
с одной и той же отметкой времени: иначе в базе окажутся пути к файлам,
которых нет в архиве, или наоборот.
