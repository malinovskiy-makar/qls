...
# ⚠️ ХЕШ КОММИТА ВШИВАЕТСЯ В ОБРАЗ, А НЕ ЧИТАЕТСЯ ИЗ .git.
# Каталога `.git` в образе нет, да и читать его из работающего контейнера
# было бы неправдой: контейнер живёт своей жизнью, а клон на сервере
# тем временем уезжает вперёд. Значение приходит АРГУМЕНТОМ СБОРКИ и
# застывает в образе — ровно тем кодом, который в нём лежит.
#
# Значение по умолчанию `unknown` намеренно: боевая сборка запускается БЕЗ
# --build-arg (docs/SERVER.md, «Обновить сайт до свежего кода»), получает
# `unknown`, и поведение боя от этой правки не меняется ничем — плашку
# окружения рисует только SITE_ENV=dev, а не наличие хеша.
ARG GIT_SHA=unknown
ENV GIT_SHA=$GIT_SHA
