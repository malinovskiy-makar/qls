...
INSTALLED_APPS = [
    ...
    'olympiads',
    # Тренажёр 1 тура олимпиады «Высшая проба» (ADR 0124): пункты теста чужого
    # формата живут отдельно от банка задач.
    'vp',
...
# Сколько секунд НЕ ходить к сервису после отказа (короткоживущий
# выключатель, `catalog/search_client.py`). Пока контейнера `search` нет —
# а на боевом сервере его нет и сегодня — без выключателя каждый поисковый
# запрос платил бы полный `SEARCH_SERVICE_TIMEOUT` за пустое ожидание.
# Ноль выключает выключатель совсем: нужно тестам, которые проверяют, что
# каждый запрос честно доходит до сети.
SEARCH_SERVICE_BREAKER_SECONDS = float(
    os.environ.get('SEARCH_SERVICE_BREAKER_SECONDS', '30').strip() or 30)
...
AI_PRICES = {
    ...
    # Прямой прайс Z.ai, тот же, что в офлайн-замере 10.09.2026
    # (reports/llm_search_eval/orclient.py). Кортеж из двух чисел: цену
    # кэша Z.ai отдельно не объявляет, и `core._cost` выводит её обычным
    # множителем от цены входа.
    'glm-5.3-flash': (0.15, 0.50),
    'glm-5.3': (1.40, 4.40),
}

# ─── Суточный денежный потолок на функцию, $ в сутки ─────────────────────
#
# Второй вид лимита рядом с AI_GENERATOR_DAILY_LIMIT и про ДРУГОЕ. Счётчик
# обращений привязан к пользователю и на гостя не работает вовсе; потолок
# защищает сайт целиком там, где функцией пользуются БЕЗ ВХОДА.
#
# ⚠️ ОТКУДА ЧИСЛО ДЛЯ ПОИСКА — ЗАМЕР, А НЕ ПРИКИДКА. Живой прогон
# 13.09.2026 на боевом ключе, путь bm25-пул → GLM-5.3-Flash: пул 50
# кандидатов, одна пачка, вход ~7,3 тыс. токенов, выход ~0,5 тыс.,
# **$0,00134 за запрос**. Когда поднимут плотную ногу, пул дорастёт до
# потолка в 150, то есть трёх пачек, — считаем по верхней границе
# ~$0,004 за запрос.
#
# $0,50 в сутки — это 370 запросов на сегодняшнем пути и 125 на полном.
# Бета — несколько репетиторов со своими учениками; столько поисков они
# руками не набирают. Потолок здесь не ради экономии, а чтобы ошибка в
# коде или чужой робот, дёргающий поиск в цикле, не выставили счёт на
# тысячи: $0,50 в сутки — это около $15 в месяц при худшем раскладе.
# Упёрлись — выдача ТИХО уходит на базовый порядок, поиск не ломается.
AI_DAILY_COST_CAPS = {
    'search_rerank': float(
    ...
    # ⚠️ ПОТОЛОК У КАЖДОЙ ПЛАТНОЙ РАБОТЫ, ДО КОТОРОЙ ДОТЯГИВАЕТСЯ САЙТ
    # (24.09.2026). До этого потолок был только у двух верхних: проверка
    # решения по фото, распознавание вложений и ИИ-сборка домашки тратили
    # сколько угодно — счётчик обращений на человека держит одного
    # пользователя, но не сотню новых аккаунтов. Упёрлись — функция
    # отвечает «на сегодня исчерпан бюджет», страница не ломается.
    # Команды данных (topic_tagging, answer_blind…) владелец запускает
    # руками со своей сметой — потолка у них намеренно нет.
    'catalog_check': float(
        os.environ.get('CATALOG_CHECK_DAILY_CAP_USD', '1.0').strip() or 1.0),
    'catalog_ocr': float(
        os.environ.get('CATALOG_OCR_DAILY_CAP_USD', '1.0').strip() or 1.0),
    'homework_plan': float(
        os.environ.get('HOMEWORK_PLAN_DAILY_CAP_USD', '1.0').strip() or 1.0),
}
...
# 17.09.2026 стояла в шапке). Поднимает владелец руками: числа считает
# человек, а не сборка — на бете важно, что версию назвали, а не то, что она
# вычислилась.
SITE_VERSION = 'Beta 1.1'
