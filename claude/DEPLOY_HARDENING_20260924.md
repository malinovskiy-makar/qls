...
⚠️ **РИСК ДЛЯ КЛАССА ЗА ОДНИМ АДРЕСОМ — решить до выкатки.** Числа взяты из
задания сессии. Школьный класс выходит в интернет с одного адреса: 30
учеников, входящих в начале урока, упрутся во вход 10/мин (+5 всплеск) —
половина получит страницу 429; 30 человек, листающих задачи, — в 60 страниц
задач в минуту на адрес. Гости класса делят и квоту разных задач 150/час.
Если бета идёт уроками — поднять `rate` у `weco_auth` и `weco_problem` в
`deploy/nginx/available/django.conf` (например 60/мин и 300/мин) или выкатить
nginx-часть позже кода.
