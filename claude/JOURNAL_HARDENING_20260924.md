...
## Фаза 4. Защита банка — В РАБОТЕ

- **4.2 (часть)** `teacher`: `visible_catalog_ids()` — `base_queryset('tutor')`
  + задачи из своих работ; `cart_items` режет до 100, `api_cart_rows` сверх
  100 — 400; `/teacher/api/problem/` через тот же шлюз (раньше без
  `content_status`). Зубастость: 3 файла к `f50266d` → 4/4 красные.
...
Фаза 4 — 4.1 решения гостю, подборки, nginx, квота задач.
