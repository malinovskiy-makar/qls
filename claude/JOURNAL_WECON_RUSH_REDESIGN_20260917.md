...
- [x] P4 Вызов дня
  - [x] P4.0 Сверка с кодом, базовый прогон (test_daily + test_sets: 50 OK)
  - [x] P4.1 Серия дней (`streak_for` и живая ячейка сделаны в P1) — тесты по спецификации
  - [x] P4.2 Страница `/game/daily/`: серия, неделя, отсчёт, карточки с топ-5 и вчерашним победителем,
        `?auto=1` без мелькания стартового экрана, телефон
  - [x] P4.3 Доска дня: без 404, «Таблица» и «Где ошиблись», своя строка, листалка, переход со второй доски
  - [x] P4.4 Живая ячейка на главной (P1) и итог раунда (P3): доска своего дня
  - [x] P4.5 ADR 0113, GAME.md, зубастость (26/26), Notion
- [x] P5 Бесконечные тесты
  - [x] P5.0 Сверка с кодом, базовый прогон (test_practice + test_page_js: 52 OK)
  - [x] P5.1 Полоса с фильтром и четырьмя счётчиками, лента пройденного со свёрткой, «Вопрос N»
  - [x] P5.2 Разбор после ответа: вердикт, решение, «Открыть задачу в каталоге», «Плохая задача?»
  - [x] P5.3 Листание и обратимый пропуск (сервер и клиент), «Закончить?», телефон
  - [x] P5.4 Экран итога: цифры, ошибки и пропуски, точность по темам, «Решать тему»
  - [x] P5.5 ADR 0114, GAME.md, зубастость (25/25), Notion
- [x] P6 Страница набора ученика и публичная страница результата
  - [x] P6.0 Сверка с кодом, базовый прогон (test_sets + test_game: 191 OK)
  - [x] P6.A1 Страница набора: приглашение и доска, переходы `/board/` и вызова дня, неверный код — 404 игры
  - [x] P6.A2 Тесты страницы набора
  - [x] P6.B1 Публичная страница результата: ник, склонение, исходы, место, «Сыграть» и «Вызвать на дуэль»
  - [x] P6.B2 Тесты страницы результата
  - [x] P6.3 ADR 0115, GAME.md, зубастость (17/17), Notion
- [x] P7 Кабинет учителя: наборы
  - [x] P7.0 Сверка с кодом, базовый прогон (`game.tests.test_sets teacher`: 69 OK)
  - [x] P7.1 Список `/teacher/game-sets/`: код с копированием, срок, прошли, средняя точность — агрегатами
  - [x] P7.2 Сборка на одном экране: режимы словами и минутами, `api/game-set/pool/` без перезагрузок,
        набор справа со стрелками, черновик в `sessionStorage`, «Открыт до», ответ без меток, телефон
  - [x] P7.3 Страница набора: плитки, «Код для класса», «Изменить срок», «Кто прошёл» со временем сдачи,
...
### P4
1. `played_days`, `streak_for` и живая ячейка «Вызов дня» были сделаны в P1 (P4.1 заранее); в P4 —
   тесты спецификации: цепочка до вчера, разрыв, два вызова в день, раунд в 00:01, аноним, best ≥ current.
2. В `config.MODES` пять режимов (есть «График» за флагом `GAME_FIGURE_ENABLED`): карточек столько,
   сколько режимов с вопросами в пуле — на бою четыре; вкладки доски дня — так же.
3. Спецификация сама себе противоречит: «0 вхождений кодов наборов в HTML» и «ссылка
   `/game/s/<код>/?auto=1`». Код остаётся в адресе ссылки, в видимом тексте его нет; тест проверяет
   разметку без `href`.
4. `?auto=1` показывал карточку набора на время запроса старта раунда (плюс разметку главной до
   исполнения скрипта в конце страницы) — теперь экран старта невидим до ответа сервера.
5. У четырёх страниц игры (`duel.html`, `duel_empty.html`, `stats.html`, `set_board.html`) стили
   лежали в `<style>` внутри `extra_style`, а база сама держит этот блок внутри `<style>`: вложенный
   тег съедал первое правило (`.du-page`, `.de-wrap`, `.st-wrap`, `.bd-wrap`). Исправлено, держит тест.
   У трёх шаблонов наборов учителя (`teacher/game_set_*`, база `teacher/base.html`) то же самое — P7.
6. Итог раунда вызова дня вёл на сегодняшнюю доску режима и для вчерашнего набора (раунд, законченный
   после полуночи) — теперь `daily.board_url(mode, gset.day)`.
7. «Макс. комбо» на досках печаталось сырым числом («×1,0», «×1,25») — теперь как в сравнении дуэли.
8. `played_pairs` делал `DISTINCT` с `created_at` из порядка модели — порядок снят (на результат не
   влиял, только на размер выборки).
9. Запросов у `/game/daily/`: гость 8, вошедший 11 (в тестовой базе три режима без вопросов, по два
   запроса каждый), доска дня 9; с 12 игроками на трёх досках — столько же.

### P5
1. `_practice_answer` отдавал `correct_choices`, `problem_id` и `solution` и на ПРОПУСК. С обратимым
   пропуском это стало бы подглядыванием (пропустил, узнал верный, вернулся и ответил) — теперь всё
   это приходит только с ответом.
2. В журнале практики не было выбранных вариантов и тем — итог не мог построить «ваш ответ» и точность
   по темам. Теперь в записи `chosen` и `topics`, и на вопрос одна запись — последний исход.
3. «В каталоге» на итоге раунда вела на `/catalog/<id>/` — такого адреса нет (404), страница задачи —
   `/catalog/problem/<id>/`. Дефект с коммита `2f450bab` («Фаза 3: экономика очков v2»), найден, когда
   ту же ссылку понадобилось построить для практики.
4. Зелёная заливка варианта под курсором в нынешнем коде уже снята (перевёрстка P2: `.opt:hover` —
   только рамка); теперь её держат тест кода и браузерная проверка.
5. Решение (`gen_solution`) есть только у сгенерированных вопросов; у вопросов банка вместо него —
   ссылка на задачу.
6. «Решать тему» возможна только для тем из окна фильтров (23 канонические): «Без темы» и
   неканонические имена фильтром не выбрать, кнопки у них нет.
7. На 1440 в ленту влезает 27 квадратиков, а не 24 из спецификации: при свёртке легенда скрыта, а
   кнопки свёртки узкие.
8. Звук больше не наезжает на «верных»: строки «Вопрос N · верных M» в полосе нет вовсе, «Вопрос N» —
   над карточкой, счётчики — в центре полосы.

### P6
1. `set_page` отдавал главную игры и для набора учителя, и для вызова дня; карточка набора жила под
   тремя зонами старта. Доска — отдельный `set_board.html` со своей вёрсткой (таблица + «На чём
   посыпались»). Неверный код — `get_object_or_404`, то есть общий 404 сайта.
2. `set_run_allowed` знает четыре отказа: дуэль без входа, «ещё не открыт», «уже закрыт», «попытка
   использована»; на странице набора они стали словами («Набор откроется …», «Набор закрыт …»,
   «Попытки закончились»). Дуэль на эту страницу не попадает.
3. `result.html` различал только `lives`; `og_title` — «N очков» без склонения; «Сыграть тоже» —
   `/game/` без режима; ника не было. `GameResult` не хранит серию верных — плитка «Макс. комбо» без
   второй строки.
4. Место на странице результата считает `leaderboard.my_row` — два запроса при любом числе игроков
   (тест держит потолок 6 запросов на страницу и ноль записей).
5. Браузерная раскладка страниц игры (`test_browser_layout`, 380 и 1280 px) нашла дефект P4: ссылка
   «Вчера никто не играл» была 28 px при правиле «элементы управления от 32 px». Исправлено.
6. Кабинет учителя ведёт на доску набора старым адресом `/game/s/<код>/board/` — он теперь переход на
   страницу набора (кабинет переделывает P7).

### P7
1. Мастер был двумя экранами: без `?mode=` — шаг 1 «Сначала режим», с ним — шаги 2–3 под пилюлями
   «1 · Режим 2 · Вопросы 3 · Название и код». Фильтры — форма `method="get"`, страницы — ссылки
   `?page=` (`Paginator` по 20): каждая перезагрузка обнуляла `picked` скрипта. Причина бага
   подтверждена по коду; бой с этой машины не проверял.
2. Шаг 1 показывал «120 с · 3 ♥ · в пуле N вопр. типа «single»», шаг 2 — «вопросы типа «numeric»»:
   внутренние имена и секунды шли в HTML из `_mode_choices`.
3. `api_game_set_fill` не знал фильтров сложности и брал `order_by('?')[:n*4]`, а тему отсеивал уже в
   Python: с редкой темой «Добрать 5» приносил меньше пяти или ничего при живых вопросах.
4. `_save_set`: попытки без верхней границы, `closes_at` не принимался. Поля `GameSet.opens_at` /
   `closes_at` уже есть — миграция не нужна.
5. Страница набора: «Публичная доска» вела на `/game/s/<код>/board/` (с P6 это переход), таблица без
   времени сдачи, «На чём посыпался класс» — по порядку и без ответов.
6. Список: `annotate(Count('results'))`, колонки «Название · Режим · Вопросов · Код · Прошло · Доска».
7. `teacher/base.html` держал `<main class="page-wrap">` без блока — у страниц наборов не было своей
   ширины; добавлен `{% block main_class %}`.
8. Найдено браузерным тестом P7: список наборов на 390 px был шириной 822 px. Невидимая подпись
   крайней правой колонки (`position: absolute`) внутри `overflow-x: auto` без `position: relative`
   вставала там, где кончается широкая таблица, и растягивала страницу. У досок игры невидимая подпись
...
### P4
- «Сыграли N» — вошедшие, как на доске; доли «Где ошиблись» — все раунды набора, и анонимные.
- Серия 0: «сыграйте любой вызов, чтобы начать серию» (в спецификации только два текста — для
  засчитанного дня и для серии, которую можно прервать); «лучшая серия» — только если она больше нуля.
- Своя строка на карточке — только если я в пятёрке; место всегда видно строкой «ваш счёт · N-е место из M».
- Вчерашний победитель — «Вчера: 1-е место – <логин> · <счёт>» (род игрока неизвестен).
- Прошедший день без набора: сводка «Классика · 10 мин» без числа вопросов — набора не было.
- Гость, сыгравший вызов: карточка «сыгран» и свой счёт без места; на доске дня — «на доске только вошедшие».
- «и ещё N» — всего минус показанные первые 10 (своя строка ниже входит в N, как в макете).
- Отсчёт, дошедший до нуля на глазах, перечитывает страницу; при убежавших вперёд часах клиента — нет.
- Телефон: одна строка лидера со «сыграли N», без «вчера»; в таблице доски нет «Макс. комбо» и
  «Чем кончилось» (не помещаются в 390 px).
- Страница вызова на 1440×800 помещается без прокрутки: «← Wecon Rush» в левой колонке верха, как в макете.
- Пять новых значков (`arrow_left`, `chevron_left`, `chevron_right`, `lock`, `info`) — в оба набора
  (`_icon.html`, `_icons.html`); кольцо медали — токен `--medal-ring`.
- Общие части доски — префикс `gb-` (у `set_board.html` префикс `bd-`, она уходит в P6).
### P5
- Итог практики живёт на экране итога раунда (`#fin-practice` рядом с `#fin-round`) из тех же карточек;
  Enter нажимает основную кнопку видимого из двух.
- Вопросы под фильтром кончились — окно «Вопросы кончились» (можно вернуться к пропущенным), а не
  сразу итог.
- Итог рисуется сразу из записей клиента и перерисовывается сводкой сервера, когда она придёт.
- Телефон: лента листается пальцем без свёртки и без «к пропущенным».
- «Поменять фильтры»: окно фильтров; «Применить» начинает новую сессию, крестик просто закрывает.
- Отвеченный вопрос из истории показывает свой разбор (вердикт и решение видны и при возврате).
- На пропущенном из истории «Вперёд» и пробел идут дальше, второй пропуск на сервер не шлют.
- Несколько верных: «Неверно. Верные ответы – 1, 2, 4.»
- Чип сложности над карточкой в практике скрыт — в макете его нет.
- Новый токен `--on-green` (номер на зелёном квадратике, обе темы) и значок `external`.
### P6
- Страница набора — только у наборов учителя; вызов дня без `?auto=1` уходит на `/game/daily/`, дуэль —
  прежняя страница игры с лобби. `?auto=1`, когда играть нельзя, тоже ведёт на страницу набора: там
  видна причина.
- «Кто прошёл · N» считает все раунды, и анонимные; своя строка анонима — по коду результата в сессии.
- Сыгравшему с оставшимися попытками кнопка называется «Сыграть ещё раз».
- В плашке — «2 минуты · 3 жизни» (число жирным, как в макете), срок — по Москве, как отсечка вызова дня.
- Подпись под таблицей набора — про анонимов («учитель их не узнает»): правило сортировки уже стоит в
  заголовке.
- Страница результата: «Вызвать <ник> на дуэль» — не себе (своему результату — «Вызвать на дуэль»);
  у сегодняшнего вызова дня — «Сыграть этот же вызов» с `?auto=1`, у прошедшего — «Сегодняшний вызов
  дня»; у результата дуэли — «Сыграть в <режим>» и строка «дуэль».
- Остаток жизней — «жизни режима − ошибки», кроме конца по жизням; число вопросов внизу — весь пул
  (`_pool_qs().count()`), как сумма вкладок главной.
- Страница результата на ПК — 1120 px, на телефоне — порядок макета: карточка, «Сыграть», дуэль с
  иконкой «Поделиться» в ряд, кривая, темы.

### P7
- Выборка пула — одна функция `pool_rows` на первую порцию страницы, `api/game-set/pool/` и «Добрать N
  случайных»; «Добрать» учитывает все фильтры, включая сложность, случайный порядок — в Python.
- Первая порция приходит в HTML (`builder_json`), дальше левый список перерисовывается из API. Набор
  справа не трогают ни фильтры, ни подгрузка; устаревший ответ API отбрасывается по номеру запроса.
- Черновик — `sessionStorage` под ключом `teacher-game-set:<режим>`: состав, название, попытки, срок;
  чистится на странице набора по `?created=1`, сообщение об успехе остаётся.
- Смена режима при непустом наборе — окно, основная кнопка «Оставить как есть»; при пустом — сразу.
- Срок: `datetime-local` читается как московское время; прошедший — «Срок уже прошёл.», нечитаемый —
  «Не получилось прочитать срок.»; попытки 1–10.
- «Изменить срок» — POST `game-sets/<код>/deadline/`; чужому набору — 403. Сама страница чужого набора —
  как раньше: переход к списку с «Это не ваш набор».
- «Где ошибся класс»: порядок ссылками `?sort=hard` / `?sort=order` (спецификация разрешает параметр),
  по умолчанию трудные сверху; ниже 40 % — красным; «Самый трудный» — наименьшая доля, при равенстве —
  меньший номер.
- Средняя точность (список и плитка) — все верные ответы / все ответы по раундам набора, а не среднее
  процентов раундов: у брошенного на втором вопросе раунда вес два ответа, а не целый раунд.
- Ответ для показа — `answer_display`: метка `(b)` / `(3)` / `a)` в начале и `;` / `.` в конце; ответ
  целиком в скобках не трогается; база не меняется.
- Подписи типа вопроса короче: «верно / неверно», «несколько верных» — как на кнопках режима в макете
...
| Фаза | Тест | Как ломал | Покраснел? |
|---|---|---|---|
| P4 | test_past_day_without_a_set_is_an_empty_board_not_404 | прошедший день без набора → 404 | да |
| P4 | test_round_saved_after_midnight_counts_for_the_day_of_its_set | серия по created_at, а не по дню набора | да |
| P4 | test_question_texts_today_only_after_own_attempt | тексты сегодня открыты всем | да |
| P4 | test_no_set_codes_in_visible_markup_and_play_links_autostart | код набора вернулся на карточку | да |
| P4 | test_second_board_address_redirects_to_the_day_board | вторая доска снова своя страница | да |
| P4 | test_own_row_is_visible_when_eleventh | своя строка на 11-м месте пропадает | да |
| P4 | test_past_day_without_a_set_is_an_empty_board_not_404 | у прошедшего дня «Сегодня… будь первым» | да |
| P4 | test_query_count_does_not_grow_with_players | имя лидера — запрос на строку | да |
| P4 | test_autostart_page_hides_the_start_screen_from_the_first_paint | автостарт без невидимости старта | да |
| P4 | test_script_lifts_the_veil_on_any_screen_change_and_on_refusal | show() не снимает невидимость | да |
| P4 | test_played_card_has_no_play_link_and_shows_score_and_place | сыгранная по базе карточка зовёт «Играть» | да |
| P4 | test_guest_meter_has_login_and_no_streak | гостю серия вместо «Войти» | да |
| P4 | test_yesterday_winner_or_nobody | «вчера» берётся из сегодняшней доски | да |
| P4 | test_day_stepper_stops_at_the_first_set_and_at_today | листалка уходит раньше первого набора | да |
| P4 | test_finish_of_yesterdays_set_leads_to_yesterdays_board | итог вчерашнего набора ведёт на сегодняшнюю доску | да |
| P4 | test_endings_are_words | «кончились жизни» подписано «вышло время» | да |
| P4 | test_endings_are_words | комбо «×1.25» с точкой | да |
| P4 | test_question_texts_of_a_closed_day_are_open_to_everyone | трудные вопросы не красные | да |
| P4 | test_chain_up_to_yesterday_counts_while_today_is_not_played | серия сгорает до полуночи | да |
| P4 | test_best_is_never_below_current | лучшая серия не считается | да |
| P4 | test_game_pages_put_bare_rules_into_extra_style | вложенный <style> вернулся | да |
| P4 | DailyBrowserTest: autostart_no_start_flash | автостарт показывает карточку набора (браузер) | да |
| P4 | DailyBrowserTest: daily_mobile_column | на телефоне топ-5 вместо одной строки | да |
| P4 | DailyBrowserTest: board_mobile_fits | на телефоне таблица шире экрана | да |
| P4 | DailyBrowserTest: daily_desktop_row_no_scroll | на ПК карточки в два ряда | да |
| P4 | DailyBrowserTest: board_desktop_two_columns | доска дня на ПК в одну колонку | да |

P4: 26 дефектов, 26 красных с первого прохода (21 проверка сервера и разметки, 5 браузерных).

| Фаза | Тест | Как ломал | Покраснел? |
|---|---|---|---|
| P5 | test_skipped_question_is_answered_later_and_the_log_keeps_one_outcome | пропуск снова навсегда (сервер) | да |
| P5 | test_round_keeps_a_skip_final | в раунде можно ответить на пропущенный | да |
| P5 | test_skip_does_not_reveal_the_answer | верный ответ уходит с пропуском | да |
| P5 | test_summary_after_answering_a_skip | в журнале два исхода на вопрос | да |
| P5 | test_summary_after_answering_a_skip | пропуски не считаются в темах | да |
| P5 | test_summary_after_answering_a_skip | ответ игрока не попадает в итог | да |
| P5 | test_generated_question_brings_its_solution_and_no_problem | решение не приходит с ответом | да |
| P5 | test_history_keeps_problem_and_solution_of_each_answer | problem_id не хранится в записи | да |
| P5 | test_word_resheno_is_gone_and_tallies_are_four | вернулось слово «Решено» | да |
| P5 | test_feedback_has_catalog_link_and_solution_and_no_topic | тема вопроса в разборе | да |
| P5 | test_right_button_is_named_by_situation | кнопка справа без «Вперёд» | да |
| P5 | test_skipped_entry_stays_answerable_and_answered_is_read_only | отвеченный из истории снова отвечается | да |
| P5 | test_hover_on_an_option_is_only_a_border | наведение заливает вариант зелёным | да |
| P5 | test_escape_and_cross_ask_to_finish | Esc в практике не спрашивает «Закончить?» | да |
| P5 | test_result_is_a_screen_with_mistakes_topics_and_actions | «Решать тему» без фильтра на тему | да |
| P5 | test_catalog_link_goes_to_the_problem_page | «В каталоге» итога раунда снова на 404 | да |
| P5 | test_misses_collect_wrong_answers_and_skips | стрелка в строке ответа — знак «→» | да |
| P5 | PracticeBrowserTest: wrong_answer_feedback_with_catalog_link | нет ссылки в каталог в разборе (браузер) | да |
| P5 | PracticeBrowserTest: skip_is_reversible | на пропущенный нельзя ответить (браузер) | да |
| P5 | PracticeBrowserTest: many_questions_fold_and_jump | лента без свёртки на 30 вопросах | да |
| P5 | PracticeBrowserTest: mobile_bottom_bar_no_side_scroll | листание на телефоне не прижато к низу | да |
| P5 | PracticeBrowserTest: desktop_no_scroll_with_solution | разбор выталкивает низ за 800 px | да |
| P5 | PracticeBrowserTest: mobile_result_no_side_scroll | итог на телефоне шире экрана | да |
| P5 | PracticeBrowserTest: hover_is_border_only | наведение заливает вариант (браузер) | да |
| P5 | PracticeBrowserTest: escape_asks_then_result_screen | Esc не ведёт к итогу (браузер) | да |

P5: 25 дефектов, 25 красных. В первом проходе три браузерных дефекта (`b_skip_locked`, `b_fold`, `b_esc`)
тоже краснили тест, но раннер падал на ожидании раньше, чем записывал проверку, и имя проверки терялось.
Раннер переделан: каждый сценарий падает сам по себе и записывает сбой в свои проверки; три дефекта
перепроверены и красные с именем проверки.

| Фаза | Тест | Как ломал | Покраснел? |
|---|---|---|---|
| P6 | test_separate_page_with_invitation_and_both_board_parts | набор снова рендерит главную игры | да |
| P6 | test_question_texts_for_the_player_and_author_only | тексты вопросов набора видны всем | да |
| P6 | test_wrong_code_is_a_game_page_with_status_404 | неверный код — общий 404 сайта | да |
| P6 | test_anonymous_rounds_are_on_the_board_as_anonymous | анонимов нет в таблице набора | да |
| P6 | test_no_play_button_when_attempts_are_spent | «Играть» при исчерпанных попытках | да |
| P6 | test_separate_page_with_invitation_and_both_board_parts | плашка срока без срока | да |
| P6 | test_board_address_redirects_and_daily_without_auto_goes_to_daily | у набора снова вторая доска | да |
| P6 | test_board_address_redirects_and_daily_without_auto_goes_to_daily | вызов дня без автостарта — главная игры | да |
| P6 | test_finish_of_a_teacher_set_leads_back_to_its_page | итог набора ведёт на старую доску | да |
| P6 | test_five_endings_have_five_words | всё, кроме жизней, — «время вышло» | да |
| P6 | test_points_are_declined | «очков» без склонения | да |
| P6 | test_nick_for_a_player_and_player_for_anonymous | ника на странице нет | да |
| P6 | test_primary_button_by_kind_of_round | «Сыграть» ведёт на главную без режима | да |
| P6 | test_place_badge_only_for_a_ranked_round | место в таблице у незачётного | да |
| P6 | test_duel_button_leads_a_guest_to_log_in_first | гость идёт в дуэль без входа | да |
| P6 | test_lives_left_when_it_can_be_computed | «осталось 0 жизней» у конца по жизням | да |
| P6 | GamePagesLayoutBrowserTest: controls_32px | ссылка «вчера» ниже 32 px (браузер) | да |

P6: 17 дефектов, 17 красных с первого прохода (16 проверок сервера и разметки, 1 браузерная раскладка).

| Фаза | Тест | Как ломал | Покраснел? |
|---|---|---|---|
| P7 | test_builder_has_no_internal_type_names_seconds_or_reloading_filters | вернул форму фильтров `method="get"` | да |
| P7 | test_deadline_is_moscow_time_and_order_is_kept | срок сохраняется как наивное время (UTC) | да |
| P7 | test_past_deadline_is_refused_and_empty_means_no_deadline | прошедший срок принимается | да |
| P7 | test_pool_api_pages_without_repeats_or_gaps_and_hides_types | в API пула внутреннее имя типа | да |
| P7 | test_pool_api_pages_without_repeats_or_gaps_and_hides_types | подгрузка повторяет вопрос | да |
| P7 | test_pool_api_refuses_students_and_guests | API пула открыт ученику и гостю | да |
| P7 | test_detail_counts_anonymous_sorts_hard_first_and_shows_when | анонимы не считаются отдельно | да |
| P7 | test_detail_counts_anonymous_sorts_hard_first_and_shows_when | по умолчанию «по порядку» | да (после усиления теста) |
| P7 | test_deadline_of_a_foreign_set_cannot_be_changed | срок чужого набора меняется | да |
| P7 | test_answer_is_cleaned_for_display_only | метка «(b)» остаётся в ответе | да |
| P7 | test_sets_list_queries_do_not_grow_with_sets | запрос на каждый набор в списке | да |
| P7 | test_builder_has_no_internal_type_names_seconds_or_reloading_filters | в подписи режима внутреннее имя типа | да |
| P7 | test_no_internal_type_names_on_the_list_and_the_set_page | на странице набора внутреннее имя типа | да |
| P7 | TeacherSetsBrowserTest: set_survives_filters_and_paging | перерисовка списка тихо переписывает набор (браузер) | да (после усиления сценария) |
| P7 | TeacherSetsBrowserTest: set_survives_filters_and_paging | смена фильтра очищает набор на экране (браузер) | да |
| P7 | TeacherSetsBrowserTest: set_restored_after_reload | набор не восстанавливается после перезагрузки | да |
| P7 | TeacherSetsBrowserTest: arrows_reorder_question_ids | стрелки не меняют `question_ids` | да |
| P7 | TeacherSetsBrowserTest: mode_switch_asks_when_not_empty | смена режима без вопроса стирает набор | да |
| P7 | TeacherSetsBrowserTest: save_clears_the_draft | черновик не чистится после сохранения | да |
| P7 | TeacherSetsBrowserTest: builder_and_detail_mobile_no_side_scroll | список наборов шире телефона | да |

P7: 20 дефектов, 20 красных. Первый проход дал 16 из 18; две проверки пришлось усилить, дефекты не
менял:
- `test_detail_counts_anonymous_sorts_hard_first_and_shows_when`: трудным был первый вопрос, и «по
  порядку» давал тот же первый ряд, что «сначала трудные». Теперь трудный — третий, и тест сверяет
  первый ряд в обоих порядках;
- браузерный `set_survives_filters_and_paging` читал счётчик и скрытое поле, а дефект из спецификации
  («перезаписывать `picked` при перерисовке списка») менял состояние скрипта, не трогая правый столбец.
  Сценарий теперь добавляет третий вопрос из нового списка и снова меняет тему: столбец рисуется из
  состояния, и тихо потерянный вопрос виден. Добавлен второй вариант дефекта — очистка набора на экране;
  три браузерных дефекта с изменёнными ожиданиями (`b_no_restore`, `b_arrows_stale`, `b_mode_no_ask`) и
  `b_draft_kept` перепроверены — красные.
...
| P3, итог фазы | `scripts/run_tests.py game`: шаг A / шаг B | 900 / 8 | OK / OK (skipped=1) |
| P4, до правок | test_daily + test_sets | 50 | OK |
| P4, новые тесты | test_daily +28 (StreakTests 7, DailyPageP4Tests 8, DayBoardP4Tests 10, AutostartTests 2, BaseStyleBlockTests 1) + браузерный DailyBrowserTest (5 проверок) | 28 + 1 | OK |
| P5, до правок | test_practice + test_page_js | 52 | OK |
| P5, новые тесты | test_practice +14 (ReversibleSkipTests 5, GeneratedSolutionTests 1, PracticeScreenSourceTests 8), test_final_screen +1 + браузерный PracticeBrowserTest (12 проверок) | 15 + 1 | OK |
| P6, до правок | test_sets + test_game | 191 | OK |
| P6, новые тесты | test_sets +10 (SetInvitationPageTests), test_result_page 8 | 18 | OK |
| P6, после фазы | test_result_page, test_game, test_sets, test_daily, test_duel, test_duel_page, test_final, браузерные layout и daily | 317 | OK (после починки ссылки «вчера») |
| P5, после фазы | test_practice, test_page_js, test_final_screen, test_round, test_final, test_start_layout, браузерные round/final/practice/modals, problems test_obzor_nav и test_palette_tokens | 190 | OK |
| P7, до правок | `game.tests.test_sets teacher` | 69 | OK |
...
### P4 (всё осталось в git до коммита P4)
- `daily.html`: разметка и стили `.dl-wrap/.dl-head/.dl-sub/.dl-timer/.dl-grid/.dl-card/.dl-meta/
  .dl-state/.dl-acts/.dl-btn/.dl-note`, строка «N вопр. · код XXXX», «Сыграно · N очков» / «Ещё не
  сыграно», примечание «На доску попадают только вошедшие…» внизу, отсчёт «ч мин с».
- `daily_board.html`: `.db-wrap/.db-sub/.db-acts/.db-btn/.db-box/table.db/.db-empty/.db-note`, «архивная
  доска», «Вчерашняя доска», «Все вызовы дня», «Сегодня этот вызов ещё никто не прошёл. Будь первым.»
  на любой день, строка «⋯» перед своей строкой.
- `views.daily_board`: `Http404('Вызова на этот день нет')`, `yesterday_url`.
- Вложенные `<style>` в `extra_style` четырёх страниц игры.
- Тест: проверка «архивная доска» (заменена на «вызов закрыт»).
### P5 (всё осталось в git до коммита P5)
- Разметка: строка полосы `.hud-practice` «Вопрос N · верных M», ряд `.practice-nav` «← Назад» /
  «Вперёд →» (знаки стрелок), окно со сводкой в одну строку (`#pr-end-ask`, `#pr-end-sum`,
  `#pr-sum-line`, `#pr-again`, `#pr-home`, кнопки «Нет» / «Да»).
- CSS: `.hud-practice`, `.practice-nav` и его кнопки, `.pr-sum-title`, `body.practice #btn-skip`.
- JS: `practiceLive`, `practiceForward`, `paintPracticeSummary` («Решено N, верных…»), прежние
  `markPractice(chosen, right)` и ветка сводки в `openPracticeEnd(summaryNow)`.
- Знак «→» в строке «ваш ответ → верный» итога раунда (стал значком).
- Тесты: точное равенство сводки из четырёх полей (стало подмножеством), проверка строки
  «Вопрос N · верных M».
### P6 (всё осталось в git до коммита P6)
- Шаблон `game/templates/game/set_board.html` целиком (доска набора: `.bd-*`, «На чём посыпались»,
  «Вопрос N» вместо текста, кнопки «Играть» / «В игру»).
- `set_board` как страница (стал переходом); `get_object_or_404` у `set_page` и `set_board`.
- `result.html`: `.facts/.fact`, «N/M», «0/3» и «–» в жизнях, подпись «время вышло» для всех исходов,
  кроме жизней, кнопка «Сыграть тоже», строка «Weconomics · Wecon Rush» вместо знака, `.tp-*`.
- `og_title` «N очков в Wecon Rush» без склонения и ника.
- Тесты: `test_board_page_opens_and_shows_questions_only_after_play` (стал переходом на страницу набора),
  проверки «Сыграть тоже» и «N очков».

### P7 (всё осталось в git до коммита P7)
- Шаг 1 мастера («Сначала режим», карточки режимов с «120 с · 3 ♥ · вопр. типа «single»») и пилюли
  «1 · Режим 2 · Вопросы 3 · Название и код».
- Форма фильтров `method="get"`, листалка `?page=` («← назад» / «вперёд →»), `Paginator` в
  `game_set_create`; ссылка «сменить режим» голым системным видом; `.sc-*` стили мастера.
- В `api_game_set_fill`: выборка `order_by('?')[:n*4]` с отсевом темы после неё, поле `text[:160]`.
- Страница набора: «Публичная доска» (`board_url`), заголовок «На чём посыпался класс», `.sd-*` прежней
  вёрстки; список: колонки «Режим», «Вопросов», «Доска».
...
- 0113 — вызов дня: серия дней и лидеры на карточке, одна доска дня без 404 (P4)
- 0114 — бесконечные тесты: разбор после ответа, обратимый пропуск, лента и итог-экран (P5)
- 0115 — страница набора ученика и публичная страница результата (P6)
- 0116 — игровые наборы учителя: сборка на одном экране, срок «Открыт до», страница набора с кодом (P7)
