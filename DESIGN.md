...
у каталога и кабинета ученика; 860 (`--w-read`) — только на четырёх страницах чтения
(домашка, разбор работы, отправленное решение, контрольная), и сужают себя
они сами, в своём `extra_style`. Страница задачи из этого списка выведена — у
неё своя полоса 1120, см. исключение ниже. Лендинг (1160), подборка (1200), подборщик (1280),
...
**Записанное исключение — страница задачи (ADR 0078, 04.09.2026; подтверждено
решением 05.09.2026 «1120 для страницы задачи — исключение из „960/860“»).**
Полоса 1120px (`.pd-wrap`) с сеткой `minmax(0, 1fr) 328px`: слева условие и решение
в ширине чтения (~760px), справа постоянно видимая карточка ИИ-чата. Ниже 980px —
одна колонка. Сторож — `config/tests/test_nav.ContentWidthTests`: страницы задачи в
его списке «чтение 860» нет. Это единственное исключение из двух ширин; второе —
повод переписать правило.
