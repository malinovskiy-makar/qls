...
подписаны со звёздочкой; `M` — оптимум монополии и монопсонии; `A`, `B`, `C` —
шаги разложения Слуцкого; `u*` — естественный уровень безработицы.

**Конфликты — не чинятся молча, решение за владельцем.**

1. `M` — ось денежной массы на денежном рынке и обозначение точки монополии
   в пяти сценах. Разделы разные, буква одна.
2. `MC` и `AC` — обозначения ориентиров регулирования в естественной монополии,
   при этом `MC` и `ATC` там же имена кривых. Двухбуквенное обозначение точки
   вдобавок выходит за правило 47.
3. `Y` — товар в блоке КПВ и выпуск в макроэкономике.
4. `М` и `П` (кириллица) — монопсонист и профсоюз в двусторонней монополии.
   Правило 47 просит латиницу, но `M` уже занят монополией, а «П» школьник
   читает сразу. Замена сделала бы подпись менее понятной — стоп-гейт правила 46
   по смыслу тот же.
5. `TP, MP, AP` — объявленное имя вертикали производственной функции (уходит
   в выгрузку). На холсте его нет: там две панели со своими заголовками, и
   величины у них разные. Один символ этой оси не описывает.
