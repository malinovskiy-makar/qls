...
def _visible_qs(user):
    ...
    # ⚠️ ГОСТЬ ВИДИТ ТОЛЬКО ОБЩИЕ СОБЫТИЯ, И ЭТА ВЕТКА СТОИТ ЯВНО.
    # Раньше аноним сюда не доходил вовсе (вью была за `@login_required`) и
    # попал бы в последнюю ветку «всё остальное» по случайности. Календарь
    # открыт гостям решением владельца 01.09.2026, и раз так — правило для
    # них должно быть написано, а не выведено из порядка веток.
    # `is_global` — это «для всех», и теперь «все» включает интернет:
    # событие с такой пометкой обязано быть публичным по смыслу (сегодня их
    # два, «ВсОШ региональный этап» и «МОШ отборочный этап»). Всё, что
    # привязано к группе или к работе, гостю не достаётся.
...
def calendar_view(request):
    """Единая страница календаря для всех ролей, включая гостя.

    ⚠️ СТРАНИЦА ПУБЛИЧНА ОСОЗНАННО (решение владельца 01.09.2026). С лендинга
    сюда ведёт карточка «Календарь олимпиадника», и гость упирался в форму
    входа. Гостю достаются только общие события — правило написано явно в
    `_visible_qs`, а не выведено из порядка веток.

    Списки `groups` и `assignments` у гостя пусты: он не попадает ни в ветку
    администратора, ни в ветку репетитора. Формы создания и правки в шаблоне
    закрыты тем же признаком (`user_role` у гостя пуст), а сами эндпоинты
    записи остаются за `@login_required`.
...
def events_api(request):
    ...
    ⚠️ ЧИТАТЬ МОЖЕТ ЛЮБОЙ, НО ВИДИТ КАЖДЫЙ СВОЁ. Отбор целиком в
    `_visible_qs`: гостю уходят только события с `is_global`. Открыт этот
    эндпоинт потому, что без него открытая гостю страница показывала бы
    пустой календарь: FullCalendar получал бы редирект на форму входа
    вместо JSON. Запись (create / update / delete) остаётся закрытой.
    """
