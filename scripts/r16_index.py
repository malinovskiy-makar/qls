# -*- coding: utf-8 -*-
"""Контактный лист снимков ревью 16.08 — одна страница на всю сессию.

    ./venv/bin/python scripts/r16_index.py

Кладёт `reports/review/index_r16.html`: снимки по фазам, в каждой строке
...
OUT = os.path.join(FOLDER, 'index_r16.html')

PHASES = {
    'ф1': ('Фаза 1', 'Пункты своей задачи доходят до всех девяти выходов'),
    'ф3': ('Фаза 3', 'Дата и время: год, границы, шлюз выдачи, время числом'),
    'ф4': ('Фаза 4', 'Один блок «Активность на сайте» вместо трёх'),
    'ф5': ('Фаза 5', 'Карточки статистики: сложность, розовая пара, знак игры'),
    'ф6': ('Фаза 6', 'Таблицы: выравнивание, порядок блоков, закреплённый столбец'),
    'ф7': ('Фаза 7', 'Мелочи: веса пунктов, сложность словами'),
}
...
def shots():
    ...
    for name in sorted(os.listdir(FOLDER)):
        match = re.match(r'^р16-(.+)-(light|dark|380)\.png$', name)
        if not match:
            continue
