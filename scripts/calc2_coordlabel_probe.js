...
const SNAP = () => {
  ...
  /* Подпись цены обязана целиком лежать левее оси, подпись количества — ниже.
     ⚠️ ПРАВИЛО ОТНОСИТСЯ ТОЛЬКО К ПОДПИСЯМ НА ГЛАВНЫХ ОСЯХ. У сцен с
     мини-панелями (дискриминация, монополист на внешнем рынке) свои оси стоят
     посреди поля построения, и их подписи по устройству лежат «внутри первой
     четверти» главных шкал. Пока панельные числа рисовались без класса, вопрос
     не вставал; с 21.08 у них есть свой класс, и правило обязано различать,
     на чьей оси стоит подпись. Признак простой: подпись у главной оси стоит
     вплотную к ней. */
  const MAIN_AXIS_PX = 26;
  const onMainAxis = (l) => (Math.abs(l.x + l.w - ox) < MAIN_AXIS_PX) ||
                            (Math.abs(l.y - oy) < MAIN_AXIS_PX + 12);
  const main = list.filter(onMainAxis);
  const inside = main.filter(l => (l.x + l.w > ox + 1) && (l.y + l.h < oy - 1));
  const rightOfAxis = main.filter(l => l.anchor !== 'middle' && l.x + l.w > ox + 1);
