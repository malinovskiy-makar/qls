...
CLAIMS = [
    ...
    # ⚠️ ПЕРЕСЧИТАНО 21.08, А НЕ ОТКЛЮЧЕНО. Прежде здесь проверялось, что запрет
    # мыши стоит одной константой (CURVE_MOUSE_DRAG = false) и что curveDraggable
    # читает её первой строкой. Обе строки удалены вместе со всем механизмом
    # перетаскивания (фаза 3.4 сессии 21.08 по прямому указанию владельца):
    # выключенная навсегда константа и функция, которая всегда отвечает «нет»,
    # это и есть тот мёртвый код, ради удаления которого фаза затевалась.
    # Запрет теперь держится ОТСУТСТВИЕМ механизма, и проверять надо его.
    # ⚠️ Ищем ОБЪЯВЛЕНИЯ, а не сами слова: в файле рядом лежит объяснение, где
    # эти имена перечислены как удалённые. Одиннадцатый по счёту случай, когда
    # проверка рискует поймать собственный комментарий.
    ('Фаза 2 · механизма перетаскивания кривых в коде нет',
     'calc2/static/calc2/30-curves.js', None,
     r'const CURVE_MOUSE_DRAG|function curveDraggable\(|function attachDrag\('),
    ('Фаза 2 · курсор над полосой захвата всегда указатель',
     'calc2/static/calc2/30-curves.js', r"\.style\('cursor', 'pointer'\)", None),
