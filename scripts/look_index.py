# -*- coding: utf-8 -*-
"""Контактный лист снимков ВИЗУАЛЬНОЙ сессии 17.08 (фазы 0–3).

    ./venv/bin/python scripts/look_index.py

Кладёт `reports/review/index_look.html`: снимки по фазам, в каждой строке
...
Снимает `node scripts/look_shots.js` — он же сверяет код ответа.
"""
import os
import re

FOLDER = os.path.join('reports', 'review')
OUT = os.path.join(FOLDER, 'index_look.html')

PHASES = {
    'ф0': ('Фаза 0. Хвост первой сессии',
           'Три числа про проверку сошлись: кнопка на карточке, пилюля над '
           'списком и колонка «Сдано работ» считают одно и то же.'),
    'ф1': ('Фаза 1. Сквозные элементы',
           'Число правится без рамки, чип стал сигналом (сложность — янтарь '
           'со звездой, решение — зелень с галкой), «Целиком» одно на всю '
           'платформу.'),
    'ф2': ('Фаза 2. Шаг «Что кладём»',
           'Пять способов набора — панели одного экрана, лента всегда из '
           'трёх шагов, поиск вдвое шире, выбранное красится, а не гаснет.'),
    'ф3': ('Фаза 3. Два экрана проверки',
           'Выделение полосой и словами, статус одной строкой, три равные '
           'секции вместо четырёх уровней карточек, кнопки балла одной '
           'группой.'),
}
...
def shots():
    ...
    for name in sorted(os.listdir(FOLDER)):
        match = re.match(r'^вид-(.+)-(light|dark|380)\.png$', name)
        if not match:
            continue
...
<title>Визуальная сессия 17.08 — снимки</title>
<style>
...
<h1>Визуальная сессия 17.08 — что смотреть глазами</h1>
