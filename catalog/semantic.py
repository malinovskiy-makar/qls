...
def embed_query(text: str) -> np.ndarray:
    ...
    ⚠️ КОДИРУЕТ ОТДЕЛЬНЫЙ ПРОЦЕСС, А НЕ ЭТОТ (С4). Раньше здесь вызывался
    `get_model()`, то есть 2,12 ГБ грузились в тот воркер, которому не
    повезло принять запрос, — и так в каждый из девяти. Это была «мина №1»,
    из-за которой смысловой поиск держали выключенным флагом. Теперь запрос
    уходит по HTTP в контейнер `search`, где модель одна на всех.

    ⚠️ ОТКАТА НА ЛОКАЛЬНУЮ МОДЕЛЬ ЗДЕСЬ НЕТ, И ЭТО ГЛАВНОЕ. Соблазн
    «если сервис не ответил — посчитаем сами» вернул бы ту самую мину, но
    уже незаметно: она срабатывала бы только когда сервис прилёг, то есть
    в худший момент. Сервис недоступен — поднимаем `SearchServiceUnavailable`,
    а деградацией до поиска по словам занимается `catalog/hybrid.py`.

    Нормализация остаётся здесь: сервис отдаёт СЫРОЙ выход модели, ровно
    такой же, какой лежит в `Problem.embedding`. Нормализовать обе стороны
    надо одинаково и по одному разу.
    """
    from catalog.search_client import encode_one

    vec = encode_one(text).astype(np.float32)
