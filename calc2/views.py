...
заменил калькулятор на Desmos: приложение graphs и страница /desmos/ удалены
2026-08-18, calc2 остался единственным.
        ...
        try:
            proc = subprocess.run(
                [binary, '-interaction=nonstopmode', '-halt-on-error',
                 # ⚠️ Запрет оболочки — ЯВНО, а не «по умолчанию выключено».
                 # У разных сборок TeX умолчания разные, а цена ошибки —
                ...
                cwd=tmpdir,
                # ⚠️ ГЛАВНАЯ ЗАЩИТА ЗДЕСЬ — ЭТИ ТРИ ПЕРЕМЕННЫЕ, А НЕ СПИСОК
                # ЗАПРЕЩЁННЫХ КОМАНД ВЫШЕ.
                #
                # Список (`_TEX_FORBIDDEN`) перечисляет команды поимённо, а
                # в TeX их сотни, и половину можно собрать из кусков. Такой
                # список закрывает то, о чём мы вспомнили, и не закрывает
                # остального: например, `\usepackage{verbatim}` плюс
                # `\verbatiminput{...}` читает чужой файл, не встречая ни
                # одного запрещённого слова.
                #
                # `openin_any=p` — это ограничение самого TeX: читать можно
                # только файлы в рабочей папке и ниже, и не скрытые. Оно
                # действует независимо от того, какой командой файл открыли.
                # `openout_any=p` — то же для записи, `shell_escape=f`
                # закрывает оболочку второй раз, уже настройкой движка.
                env={
