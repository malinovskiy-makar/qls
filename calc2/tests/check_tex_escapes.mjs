  ...
  lines.forEach((line, i) => {
    ...
    for (const lit of literals) {
      ...
      /* Внутри литерала правильный слэш записан парой. Ищем нечётную серию.
         ...
         такие записи. */
      /* ⚠️ ОСВОБОЖДАЕМ РОВНО ДВЕ ЗАКОННЫЕ ESCAPE JS: `\uXXXX` и `\xXX`.
         Среди команд LaTeX таких нет, поэтому спутать их не с чем. Проверка
         ловила «'\uE001'» как съеденную команду: после слэша идут `u` и `E`,
         то есть формально две буквы, хотя это шестнадцатеричный код символа.

         ОДНОБУКВЕННЫЕ escape НЕ освобождаем никогда — на них класс и стоит:
         `\b` съедает «\beta» (забой плюс «eta», ровно та находка, ради
         которой проверка написана), `\f` — «\frac», `\t` — «\text» и
         «\times», `\n` — «\ne», `\r` — «\right», `\v` — «\vec».
         Смотрим КАЖДОЕ вхождение, а не первое: в одном литерале может стоять
         и законный код символа, и съеденная команда. */
      const re = /(^|[^\\])\\(?:[a-zA-Z]{2,}|,)/g;
      let hit = false, mm;
      while ((mm = re.exec(lit)) !== null) {
        const rest = lit.slice(mm.index + mm[1].length);
        if (/^\\u\{?[0-9a-fA-F]{1,6}\}?/.test(rest)) continue;
        if (/^\\x[0-9a-fA-F]{2}/.test(rest)) continue;
        hit = true; break;
      }
      if (!hit) continue;
