...
const SWEEP = `(function () {
  ...
  PLACES.forEach(function (pl) {
    ...
    while ((n = walker.nextNode())) {
      ...
      /* ⚠️ ПУСТОЙ УЗЕЛ ПРОПУСКАЕМ, А НЕ ВЫХОДИМ ИЗ ОБХОДА.
         Здесь стояло «return void 0» внутри while — а это выход из ВСЕЙ
         функции обратного вызова forEach, то есть обход места обрывался на
         первом же переносе строки в разметке. Прибор поэтому почти не видел
         текстовых узлов панелей и печатал заниженное число: замер 24.08 после
         починки дал 426 случаев там, где раньше выходил ноль. */
      if (!t) continue;
