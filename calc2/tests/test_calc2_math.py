...
# docs/TESTING.md): класс поднимает НАСТОЯЩИЙ веб-сервер
# (StaticLiveServerTestCase) и запускает против него браузер отдельным
# процессом node. Три вещи, каждой из которых хватает поодиночке:
#   1. живой сервер слушает порт, а браузер ходит по `live_server_url` —
#      это внешний ресурс, который нельзя поделить между воркерами;
#   2. раннер запускает Chromium, и восемь Chromium разом на восьми
#      воркерах душат машину, отчего раннер выходит за таймаут 180 с и
#      тест «падает» по причине, не имеющей отношения к calc2;
#   3. LiveServerTestCase обслуживает запросы браузера в отдельном потоке,
#      который ходит в базу; под параллельной нагрузкой это уже давало
#      плавающий пропуск «calc2 не загрузился» (карточка в Notion, один
#      раз из трёх ПОСЛЕДОВАТЕЛЬНЫХ прогонов — параллельность сделает хуже).
