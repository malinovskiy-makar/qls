...
/* Текст подписи целиком, но без подсказки. У подписи бывает дочерний <title>
...
await t('машина сама подписала максимум, минимум и перегиб', () => page.evaluate(() => {
  ...
  /* Н25: две однотипные величины выводятся парой в скобках, а не через запятую
     двумя равенствами. Подпись называет обе: где и сколько. */
  return (has(/^max: \(x[*∗]; y[*∗]\) = \(/) && has(/^min: \(x[*∗]; y[*∗]\) = \(/)
          && has(/^перегиб: \(x[*∗]; y[*∗]\) = \(/))
    || names.join(' | ');
}));
