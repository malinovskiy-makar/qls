  ...
  },
  {
    ...
    run: `resetSceneMemory(); pickScene('sd');
          ...
          return { onQuad: onQuad, offQuad: offQuad, qmin: qmin, pmin: pmin,
                   ...
                   mathBelow: mathBelow, mathAny: mathBelow > 0 ? 1 : 0,
                   mathFrac: (best && best.pts.length) ? mathBelow / best.pts.length : -1,
                   mathErr: best ? best.err : 99 };`,
    checks: [['с галочкой точек с P < 0', 'onQuad', 0, 0],
             ...
             /* ⚠️ ЧИСЛО 79 БЫЛО ОТПЕЧАТКОМ ДЕФЕКТА, А НЕ ПРАВИЛОМ (01.09).
                Проверка брала путь с наименьшей невязкой и считала его точки.
                До правки панелей ПОБЕЖДАЛА ПОЛОСА ЗАХВАТА кривой (241 точка,
                79 ниже нуля): она рисовалась по шкалам ВСЕГО холста, а `ms`
                здесь считался ими же — две ошибки совпадали, и путь сходился.
                Настоящая кривая при этом лежала в верхней панели и невязку
                давала большую. Сейчас полоса и кривая рисуются одними шкалами,
                побеждает сама кривая (485 точек), и 167 из них ниже нуля.
                Правило же было и остаётся другим: у «Математики» отрицательная
                часть параболы СТРОИТСЯ, и строится ровно там, где парабола
                отрицательна. Долю и стережём — она не зависит ни от густоты
                сетки, ни от того, чей путь победил: x² − 4 отрицательна при
                |x| < 2, то есть на трети окна −6…6. */
             ['в «Математике» отрицательные точки есть', 'mathAny', 1, 0],
             ['их доля = доле участка, где парабола ниже нуля', 'mathFrac', 1 / 3, 0.03]],
  },
  {
