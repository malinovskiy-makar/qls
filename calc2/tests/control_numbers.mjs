...
head('Сессия 02.09 · монополист и внешний рынок: ОДИН график');
/* Проверяется НАБЛЮДАЕМОЕ: сцена рисует один график на общем выпуске, на нём
   четыре линии (D, MR, MC, Pw) и отметки, а табло говорит своё. Число узлов и
   порядок элементов не закрепляются.

   ⚠️ Здесь стерёгся реестр из ДВУХ мини-панелей. Правило не изменилось —
   «сцена заводит ровно столько панелей, сколько рисует», — изменилось само
   число: графиков стало один, и панель у сцены общая, 'main'. Проверка нужна
   ровно за тем же: точки, площади и ключевые точки не должны считаться по
   чужим шкалам. У «Дискриминации 3-й степени» панелей по-прежнему две, и её
   проверки не тронуты (panels_probe.mjs). */
const D3 = `
  var seen = function (s) {
    return [].slice.call(document.querySelectorAll('#chart text'))
      .some(function (e) { return (e.textContent || '').indexOf(s) >= 0; }) ? 1 : 0;
  };
  /* Подписи линий читаем по data-raw, а не по textContent: рядом с набранной
     формулой KaTeX держит невидимую копию для чтецов экрана, и «MC» из неё
     читается утроенным. */
  var raws = function () {
    return [].slice.call(document.querySelectorAll('#chart [data-raw]'))
      .map(function (e) { return e.getAttribute('data-raw'); });
  };
  var has = function (s) { return raws().indexOf(s) >= 0 ? 1 : 0; };
  var lines = function () {
    return ['D', 'MR', 'MC', 'P_w'].filter(function (s) { return has(s); }).length;
  };
...
`;
for (const [tag, d2, mc, wQ, wq1, wP1, wq2, wBand] of [
  ['норма Pw = 50, MC = Q', '50', 'Q', 50, 25, 75, 25, 'Экспорт = 25'],
  ['Pw = 120 выше резервной цены', '120', 'Q', 120, 0, null, 120, 'Экспорт = 120'],
  /* Мировая цена НИЖЕ предельных издержек в оптимуме: вывозить невыгодно, и
     фирма ведёт себя как обычный монополист внутри страны. Экспорт здесь
     РОВНО ноль, а не отрицательное число: q₂ = Qtot − q₁, и решатель отдаёт
     весь выпуск дому сам. Отрезка экспорта на холсте при этом нет вовсе. */
  ['Pw = 10 ниже издержек', '10', 'Q', 33.3333, 33.3333, 66.6667, 0, '']]) {
  r = await run(D3 + `world('100 - Q', '${d2}', '${mc}');
    var d = STATE.discr3 || {};
    return { found: d.found ? 1 : 0, Qtot: d.Qtot, q1: d.q1, P1: d.P1, q2: d.q2,
             lines: lines(), pans: pans(), noHome: told('рынка нет'),
             band: seen('Экспорт = ') ? 1 : 0,
             m: has('M') };`);
  ...
  cmp(tag + ': четыре линии на одном графике', r.lines, 4, 0);
  cmp(tag + ': реестр панелей', r.pans, 'main', 0);
  cmp(tag + ': отрезок экспорта', r.band, wBand ? 1 : 0, 0);
  cmp(tag + ': точка внутреннего оптимума', r.m, wq1 > 0 ? 1 : 0, 0);
}
...
r = await run(D3 + `world('100 - Q', '50', '5');
  ...
  return { found: d.found ? 1 : 0, unb: d.unbounded ? 1 : 0, q1: u.q1, P1: u.P1, Pw: u.Pw,
           lines: lines(), note: seen('Объём экспорта не ограничен'), pans: pans(),
...
cmp('MC = 5 ниже Pw: график нарисован целиком', r.lines, 4, 0);
cmp('MC = 5 ниже Pw: реестр панелей', r.pans, 'main', 0);
cmp('MC = 5 ниже Pw: подпись на линии мировой цены', r.note, 1, 0);
cmp('MC = 5 ниже Pw: табло говорит «не ограничен»', r.says, 1, 0);

head('Сессия 02.09 · ключевые точки одного графика');
/* Раньше здесь сверялось, что точки ЛЕВОЙ панели не утекли в ПРАВУЮ. Панель
   теперь одна, и утекать некуда — но смысл проверки тот же: на общей оси Q
   стоят ОБЕ отметки сцены со своими проекциями, и обе читаются по её шкалам. */
r = await run(D3 + `world('100 - Q', '50', 'Q');
  var L = keyTargets('main');
  var at = function (x, y) { return L.some(function (p) {
    return Math.abs(p.x - x) < 0.5 && Math.abs(p.y - y) < 0.5; }) ? 1 : 0; };
  var proj = L.filter(function (p) { return /^проекция/.test(p.name); })[0];
  return { m: at(25, 75), mx: at(25, 0), my: at(0, 75),
           tot: at(50, 50), tx: at(50, 0), ty: at(0, 50),
           pans: pans(), name: proj ? proj.name : '' };`);
cmp('внутренний оптимум (25; 75)', r.m, 1, 0);
cmp('его проекция (0; 75)', r.my, 1, 0);
cmp('его проекция (25; 0)', r.mx, 1, 0);
cmp('общий выпуск (50; 50)', r.tot, 1, 0);
cmp('его проекция (0; 50)', r.ty, 1, 0);
cmp('его проекция (50; 0)', r.tx, 1, 0);
cmp('панель у точек одна', r.pans, 'main', 0);
...
/* ================================================================
   ...
   ⚠️ Площади меряются У НАРИСОВАННОГО (тот же приём, что в ADR 0086):
...
/* ================================================================
   ...
   Долг ADR 0086 закрыт: обычная монополия, потолок, пол, квота, ставка,
...
/* ⚠️ Числа названы поимённо, а не проверены неравенством «VC > 0». У пола 80 и
   ...
   MC и MC ± ставка лежат деньги бюджета, они в излишек не входят (ADR 0086). */
