  ...
  if (STATE.mathSub === 'tangent') {
    if (isNaN(r.y0)) html = '<div class="warn">В этой точке функция не определена.</div>';
      ...
      html += '<div class="sb-note"><b>Как это получилось</b>'
        ...
        + `Предельные издержки это производная общих издержек, предельная выручка — производная `
        + `выручки, предельный продукт — производная выпуска по труду. Поэтому фраза «оптимум там, `
        + `где $MR = MC$» — это то же самое, что «производная прибыли обратилась в ноль».</p>`
        + `<p><b>Вывод.</b> Наклон касательной, скорость изменения и предельная величина — три `
    ...
    html += '<div class="sb-note"><b>Как это получилось</b>'
      ...
      + 'яркая — та же функция, пропущенная через новое правило. Сравнивать их надо по точкам: '
      ...
      + 'нуля ничего не даёт — по ним и видно, вокруг чего идёт растяжение.</p>'
    ...
    if (r.error) html = `<div class="warn">${r.error}</div>`;
      ...
      html += '<div class="sb-note"><b>Как это получилось</b>'
        ...
        + 'Поэтому касание — признак оптимума внутри области, а не закон на все случаи.</p>'
...
function updateGraphPanel() {
  ...
  if (!shown.length) {
    box.innerHTML = '<div class="muted">Введите формулу в поле слева — здесь появятся нули функции, '
