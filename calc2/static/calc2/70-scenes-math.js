...
function mathOptimumReasoning(r) {
  ...
  h += '<p>Экстремум там, где касательная горизонтальна, то есть f′(x) = 0. '
     + 'Корни ищутся численно: сетка по отрезку и уточнение делением пополам, '
     + 'поэтому подходит любая функция, даже кусочная.</p>';
  if (!ext.length) h += '<p>На этом отрезке производная в нуль не обращается: ни максимумов, ни минимумов.</p>';
  ext.forEach(p => {
    const s = d2Num(f, p.x);
  ...
  if (STATE.mathSub === 'tangent') {
    if (isNaN(r.y0)) html = '<div class="warn">В этой точке функция не определена.</div>';
      ...
      html += '<div class="sb-note"><b>Как это получилось</b>'
        + `<p>Производная это скорость: на сколько меняется y, если x подвинуть чуть-чуть. `
        ...
        + `<p>Треугольник у точки и есть это отношение: горизонтальный катет Δx, `
        ...
        + `где ниже — убывает, а её собственные нули это максимумы и минимумы самой функции.</p>`
        + '</div>';
    }
    ...
    if (r.error) html = `<div class="warn">${r.error}</div>`;
      ...
      html += '<div class="sb-note"><b>Как это получилось</b>'
        + `<p>В каждой точке x берётся ${r.isMin ? 'наименьшее' : 'наибольшее'} из значений всех функций. `
        ...
        + `<p>Точки перелома это корни разности пары функций: где f₁ − f₂ меняет знак, там кривые и меняются местами. `
    ...
    if (r.error) html = `<div class="warn">${r.error}</div>`;
      ...
      html += '<div class="sb-note"><b>Как это получилось</b>'
        + '<p>Ограничение это кривая: точки, где g(x, y) обращается в ноль. '
        + 'Она ищется численно, для каждого x подбирается свой y, поэтому подходит '
        + 'любая форма, а не только прямая.</p>'
        + `<p>Вдоль этой кривой перебирается значение цели, и берётся ${r.wantMax ? 'наибольшее' : 'наименьшее'}. `
        + 'Тонкими линиями нарисован веер линий уровня цели, жирной — та, что проходит через найденную точку.</p>'
        + '<p>Она касается ограничения, а не пересекает его. Это и есть признак оптимума: '
