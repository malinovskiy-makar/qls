...
function piecewiseNodesQ(curve, lo, hi) {
  ...
  if (curve.linear && isFinite(curve.linear.a) && isFinite(curve.linear.b)) breaks = [];
    ...
    /* ⚠️ У ПРЕДЛОЖЕНИЯ ПРАВОГО КОНЦА НЕТ, И ЭТО НЕ ОШИБКА ДАННЫХ.
       `sumDomainTo` там равен бесконечности намеренно (см. sumLinearRecord):
       верхней границы у суммарного предложения не существует, линия идёт до
       края кадра при любом отдалении. Узлом бесконечность быть не может —
       спрашиваем конечность явно, а не полагаемся на то, что её отсеет
       фильтр ниже: там правило про «внутри окна», а здесь про «узел есть». */
    if (isFinite(curve.sumDomainTo) && curve.sumDomainTo > 0) breaks.push(curve.sumDomainTo);
  }
...
/* ── СЛАГАЕМОЕ РЯДОМ СО СВОЕЙ СУММОЙ — ПО ОБРАЗЦУ СЛОЖЕНИЯ КПВ ─────────
   Владелец 25.08: «график всё ещё выглядит переполненным; сделай оформление
   так же, как при сложении нескольких КПВ». Образец — `54-scenes-ppf.js`,
   drawPpfSumCurves: слагаемое рисуется толщиной 1,6 штрихом «5 4», сумма —
   2,8 сплошной. Толщину мы уже взяли раньше; штриха не было, и семь сплошных
   линий одного веса читались как семь равноправных.

   ⚠️ ПРОЗРАЧНОСТЬ 0,6 ИЗ ОБРАЗЦА НЕ КОПИРУЕТСЯ — ОНА НЕ ДЕРЖИТ НАШУ НОРМУ.
   По решению владельца прозрачность взята НАИБОЛЬШАЯ ИЗ ПРИГЛУШАЮЩИХ: перебор
   с шагом 0,05 от 0,5 вверх по каждому из восьми цветов палитры и в обеих
   темах, для каждого — первое значение, при котором контраст ИТОГОВОГО (уже
   смешанного с холстом) цвета к холсту доходит до 3:1; из шестнадцати
   найденных берётся самое большое, иначе какой-нибудь цвет норму не удержит.

     цвет            светлая  тёмная        цвет            светлая  тёмная
     фиолетовый        0,70    0,95         маджента          0,70    0,95
     бирюзовый         0,80    0,75         сиреневый         0,85    0,80
     янтарный          0,90    0,70         сланцевый         0,75    0,90
     зелёный           0,80    0,75         пыльная роза      0,85    0,75

   Итог — 0,95 (его требуют фиолетовый и маджента в тёмной теме); при нём
   худший контраст 3,04:1. И это ровно то, что замерила прошлая сессия:
   приглушать прозрачностью здесь нечем, 0,95 от единицы глазом не отличить.
   Поэтому вес слагаемого несут ШТРИХ и ТОЛЩИНА, а прозрачность стоит той,
   какую назвал перебор, и ничего собой не решает.

   ⚠️ ДВА ПУНКТИРА В ОДНОЙ СЦЕНЕ, И ОНИ РАЗВЕДЕНЫ ТРЕМЯ ПРИЗНАКАМИ.
   У суммарной кривой есть участок «рынка здесь нет» — штрих 12/6 при полной
   толщине 3,2 и полной непрозрачности. У слагаемого штрих 5/4 при толщине
   1,6. Отличаются длиной штриха вдвое с лишним и толщиной ровно вдвое, то
   есть спутать их нельзя даже на чёрно-белой печати. */
const SUM_GROUP_DASH = '5 4';
const SUM_GROUP_OPACITY = 0.95;

// Штрих кривой: у слагаемого сложения — мелкий, у остальных его нет.
function curveDash(curve) {
  if (typeof sumSceneOn === 'function' && sumSceneOn() && curve && curve.sumGroup && curve.kind !== 'sum') {
    return SUM_GROUP_DASH;
  }
  return null;
}

// Прозрачность кривой: приглушено только слагаемое сложения.
function curveOpacity(curve) {
  if (typeof sumSceneOn === 'function' && sumSceneOn() && curve && curve.sumGroup && curve.kind !== 'sum') {
    return SUM_GROUP_OPACITY;
  }
  return 1;
}
  ...
  STATE.curves.forEach(curve => {
    ...
    if (ghostTo > 1e-9) {
      ...
      const path = g.append('path').datum(pts)
        ...
        .attr('d', line);
      // Слагаемое сложения: мелкий штрих и приглушение (см. curveDash / curveOpacity).
      const dash = curveDash(curve), op = curveOpacity(curve);
      if (dash) path.attr('stroke-dasharray', dash);
      if (op < 1) path.attr('opacity', op);
    }
