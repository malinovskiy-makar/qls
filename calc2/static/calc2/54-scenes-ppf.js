...
function updatePpfPanel() {
  ...
  html += '<p>Граница возможностей это все наборы, при которых ресурс израсходован '
  ...
  html += '<p>Альтернативные издержки это наклон касательной в точке. Берём две близкие '
  ...
  if (linear) {
    html += '<p>Кривая прямая, поэтому наклон один и тот же везде: за каждую единицу X '
    ...
    html += '<p>Наклон меняется вдоль кривой, поэтому и издержки не одни и те же:</p>';
...
function updatePpfSumPanel() {
  ...
  html += '<div class="sb-note"><b>Как это получилось</b>';
  html += '<p>Складываем не по вертикали, а по Минковскому: для каждого суммарного выпуска '
  ...
  if (ord.length) {
    html += '<p>Порядок специализации задают альтернативные издержки $X$: первым наращивает '
...
function updatePpfTradePanel() {
  ...
  html += '<div class="sb-note"><b>Как это получилось</b>';
  html += `<p>Внутренняя цена X это альтернативные издержки: сколько Y страна теряет за `
  ...
  if (d.xp != null) {
    if (/касание/.test(d.regime)) {
      html += `<p>КПВ вогнутая, поэтому специализация неполная: страна наращивает X, пока `
      ...
      html += `<p>КПВ прямая: издержки не растут, поэтому выгодно уйти в специализацию целиком. `
...
function updateTradeBPanel() {
  ...
  html += `<p>Страна ${tbName(d.lowIdx)} специализируется на X и меняет его на Y по мировой цене `
  ...
  html += `<p>Но взять Y больше, чем партнёр вообще выпускает, неоткуда. У ${tbName(d.highIdx)} `
  ...
  html += '<div class="hint" style="margin-top:8px;padding-top:8px;border-top:.5px solid var(--border);">' +
    ...
    `Отсюда сразу следуют границы. При Pw&nbsp;≤&nbsp;${fmt(d.bL)} стране&nbsp;${d.lowIdx} невыгодно продавать X ` +
    '(дома за него дают не меньше), и избыточное предложение обнуляется. ' +
    `При Pw&nbsp;≥&nbsp;${fmt(d.bH)} стране&nbsp;${d.highIdx} невыгодно покупать X (дешевле произвести самой), и ` +
