...
function renderCurveList() {
  ...
  STATE.curves.forEach(curve => {
    ...
    if (fInp) {
      // Имя-подпись рядом с формулой было бы дублем: формула и есть подпись.
      top.replaceChild(fInp, nm);
...
const FORMULA_EXAMPLES = {
  ...
  PPF: { title: 'Граница возможностей Y = f(X)', items: [
    ['100 - X',          'линейная, альт. издержки постоянны'],
    ['sqrt(10000 - X^2)', 'дуга, альт. издержки растут'],
    ['100 - 0.01*X^2',   'парабола, альт. издержки растут'],
    ['100 - 10*sqrt(X)', 'выпуклая, альт. издержки убывают'],
  ] },
