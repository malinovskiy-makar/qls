...
function eqSearchSpan(D, S) {
  /* Край окна округляем ВВЕРХ до степени двойки. Панорама двигает его на
     единицу-другую за кадр, и без округления отрезок поиска — а с ним и ключ
     кэша аналитики — менялся бы на каждом кадре, то есть кэша не было бы
     вовсе. Обещание «искать не меньше, чем видно» округление вверх только
     усиливает, а на ответ не влияет: корень уточняется бисекцией, а не
     берётся из узла сетки. */
  const floor = Math.pow(2, Math.ceil(Math.log2(Math.max(100, CONFIG.Qmax))));
  let far = 0;
  [D, S].forEach(c => { const z = curveZeroQ(c, floor * 4); if (z > far) far = z; });
