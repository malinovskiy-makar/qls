...
/* ОБЪЁМ СТОРОНЫ ПРИ ЗАДАННОЙ ЦЕНЕ — один помощник на все сюжеты с ценой
   ...
   нет» (ADR 0085). Второго механизма не заводим. */
...
function interventionKeyValues() {
  ...
  if (STATE.taxActive && STATE.taxEq) {
    ...
    return `<div class="stat"><span>$Q$ (объём торговли)</span><b>${fmt(t.Q)}</b></div>` +
      ...
      `${sub ? 'субсидию' : 'налог'}, и одной цены на этом рынке больше нет. ` + was + `</div>`;
  }
...
function updateTaxPanel() {
  ...
  if (noBase) {
    ...
    html += `<div class="hint" style="margin-top:6px;">Столбца «до» здесь нет, и это не пропуск:
      ...
      ${isSub ? 'выигрыш' : 'бремя'} сторон не считаются: обе величины дают разность с исходным состоянием.</div>`;
  } else {
  ...
  if (adv) {
    ...
    html += `<div class="hint" style="margin-top:6px;">${pf.label}: ставка берётся долей от ` +
      `${baseName}. Кривая после это ${pf.curve}, то есть S<sub>после</sub> = ${fmt(k)}·S. ` +
