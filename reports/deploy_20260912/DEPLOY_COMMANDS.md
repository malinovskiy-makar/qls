...
Хеш здесь намеренно не записан: он менялся бы при каждой правке этого
же файла. Сверяйте с локальным — на машине владельца

```bash
git -C /c/Users/shipu/qls log -1 --pretty='%h %s' main
```

печатает ровно то, что должно оказаться на сервере. Если
