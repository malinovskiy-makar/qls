...
**185 коммитов, 353 файла, +118 607 / −4 310** относительно `origin/main`.
Три направления:
...
⚠️ **Сверять надо ДВА вывода, и вторая команда — НЕ на сервере.** Хеш в
этом файле намеренно не записан: он менялся бы при каждой правке самого
файла. Поэтому сверка идёт так: команда выше печатает хеш на сервере,
а команда ниже — хеш на машине владельца, и запускать её надо **в
отдельном окне PowerShell, не внутри сессии SSH** (на сервере такого
пути нет, и `git -C` честно ответит `No such file or directory`).

```bash
git -C C:/Users/shipu/qls log -1 --pretty='%h %s' main
```

Два хеша совпали — код доехал. Если
