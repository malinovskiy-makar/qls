...
def choose_best(members):
    ...
    2. **Ровно одна approved.** Она фаворит — кроме ДВУХ исключений, и в
       обоих группа уходит на глаз, а фаворита не получает никто:

       * у approved нет картинки, а у кого-то из двойников есть: картинка в
         условии несёт содержание, и потерять её молча нельзя;
       * у approved пуст `answer`, а у кого-то из двойников он непуст. Это
         не про полноту: **задачу с пустым ответом нельзя дать ученику**.
         Правило действует для двойника ЛЮБОГО источника, а не только
         SolveHub и Школково.

       Исключения считаются по отдельности и разными причинами
       (`approved_picture_review` и `approved_answer_review`). Если
       подходят оба, записывается причина про картинку; сколько таких
       групп — команда печатает отдельной строкой, чтобы ни одно из двух
       чисел не читалось как полное.
    ...
    if len(approved) == 1:
        фаворит = approved[0]
        прочие = [m for m in members if m['id'] != фаворит['id']]
        if фаворит['figures'] == 0 and any(m['figures'] > 0 for m in прочие):
            return DupMark.Rule.APPROVED_PICTURE_REVIEW, None
        if not (фаворит['answer'] or '').strip() and any(
                (m['answer'] or '').strip() for m in прочие):
            return DupMark.Rule.APPROVED_ANSWER_REVIEW, None
