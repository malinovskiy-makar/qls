...
def _student_payload(user, period):
    ...
    return {
        # ⚠️ ПРОФИЛЬ НУЖЕН ШАПКЕ СТРАНИЦЫ, а не расчётам. Шапка на всех
        # четырёх вкладках профиля одна и та же (`_profile_header.html`), и
        # любая другая её высота вернула бы прыжок полосы вкладок.
        # Берётся через getattr с запасным значением — тем же способом, что
        # в `config/context_processors.py::_avatar_url`: у части старых
        # аккаунтов профиля нет вовсе, и падать шапка не должна. Плюс это
        # НЕ лишний запрос: связь один-к-одному кэшируется на объекте, и
        # шапка сайта потом читает уже готовое. Создавать профиль на
        # GET-запросе ради подписи нельзя.
