...
class Achievement(models.Model):
    ...
    def rarity_percent(self):
        ...
        * число > 0 — награда у части учеников;
        * 0.0 — учеников с опытом есть, а награда не у кого: ЗАКОННЫЙ ноль;
        * None — учеников с опытом нет вовсе, считать не на ком («нет данных»).
        Ноль здесь утверждает «никто не получил», а None ничего не утверждает;
        на экране первое печатается «этого добились 0%», второе не печатается
        вовсе (иначе на пустой платформе даже полученная награда «редчайшая»).
        """
        shares = rarity_map()
        if shares is None:
            return None
        return shares.get(self.pk, 0.0)


# ---------------------------------------------------------------------------
# Витринные (демо) аккаунты — ЕДИНСТВЕННЫЙ источник правды об их опознании
# ---------------------------------------------------------------------------
# Команда `seed_showcase_student` заводит на боевой платформе демо-ученика с
# опытом и наградами. Живые люди не должны видеть, что этот ученик изменил
# «этого добились N %» у их наград, поэтому редкость его не считает — ни в
# числителе, ни в знаменателе.
#
# Правило живёт ЗДЕСЬ, а не в команде, потому что импортировать management-
# команду из модели нельзя (слой моделей не знает про команды), а команда
# модель импортировать вправе: она берёт правило отсюда и падает, если её
# логины под него не подходят. Опознание — по логину, а не по новому полю:
# миграция ради этого не нужна.
#
# Правило нарочно без учёта регистра: Django-логины регистрозависимы, а
# `startswith` на SQLite сравнивает без учёта регистра, на PostgreSQL — с
# учётом. Чтобы исключение не зависело от того, на какой базе оно считается,
# оба способа (питон и запрос) везде смотрят одинаково.
SHOWCASE_LOGIN = 'demo'
SHOWCASE_PREFIX = 'demo-'


def is_showcase_login(username):
    """Витринный ли логин: равен `demo` либо начинается на `demo-`."""
    name = (username or '').lower()
    return name == SHOWCASE_LOGIN or name.startswith(SHOWCASE_PREFIX)


def showcase_q(prefix=''):
    """То же правило как условие запроса. `prefix` — путь до пользователя:
    'user__' для моделей с полем `user`, '' для самого User."""
    return (models.Q(**{f'{prefix}username__iexact': SHOWCASE_LOGIN})
            | models.Q(**{f'{prefix}username__istartswith': SHOWCASE_PREFIX}))
...
def _count_rarity():
    """(знаменатель, {id достижения: доля, %}) — без витринных аккаунтов.

    Фильтр `real` ОДИН и передаётся в оба запроса: знаменатель и числитель не
    могут разъехаться при следующей правке (демо в одном есть, в другом нет —
    и проценты поползли бы за сто)."""
    real = ~showcase_q('user__')
    total = StudentProgressProfile.objects.filter(real, xp_total__gt=0).count()
    if not total:
        return 0, {}
    rows = (EarnedAchievement.objects.filter(real).values('achievement_id')
            .annotate(owners=models.Count('user', distinct=True)))
    return total, {row['achievement_id']: round(row['owners'] * 100.0 / total, 1)
                   for row in rows}


def rarity_map():
    """{id достижения: доля учеников, %} — ОДНИМ запросом на всю страницу, либо
    None, если считать не на ком.
    ...
    редчайшей. Витринные аккаунты (`is_showcase_login`) не считаются нигде.

    ⚠️ НОЛЬ И «НЕТ ДАННЫХ» — РАЗНЫЕ УТВЕРЖДЕНИЯ. Пустой словарь или 0.0 при
    отсутствии учеников с опытом сказал бы «эту награду не получил никто», а
    это неправда: получивших нет потому, что учеников нет. Поэтому при пустом
    знаменателе возвращается None, а 0.0 остаётся только законным «ученики
    есть, награда ни у кого» (у `rarity_percent`).

    Кэш хранит пару (знаменатель, карта), а не голую карту: так «нет данных»
    кэшируется как есть (не как ноль и не как промах), а запись прежнего
    формата (до правки 21.09.2026 там лежал сам словарь, а пустое состояние — `{}`,
    которое читалось бы как «у всех 0 %») просто пересчитывается.
    """
    cached = cache.get(RARITY_CACHE_KEY)
    if not isinstance(cached, tuple):
        cached = _count_rarity()
        cache.set(RARITY_CACHE_KEY, cached, RARITY_CACHE_SECONDS)
    total, shares = cached
    return shares if total else None
