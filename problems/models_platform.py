...
class UserProfile(models.Model):
    ...
    class Level(models.TextChoices):
        ...
        22.09.2026: подписи стали короткими словами («Новичок», «Профи»), а
        длинное объяснение ушло в `LEVEL_HINTS` — в списке оно стоит второй
        строкой под названием. КОДЫ ПРЕЖНИЕ, данные не переписываются.
        """
        NOVICE = 'novice', 'Новичок'
        BASIC = 'basic', 'Начинающий'
        REGION = 'region', 'Продолжающий'
        FINAL = 'final', 'Профи'

    # Описание уровня — рядом с самим уровнем, чтобы не разъехалось.
    LEVEL_HINTS = {
        'novice': 'Олимпиадной экономикой пока совсем не занимался',
        'basic': 'Уже дошёл до школьного или муниципального этапа ВсОШ',
        'region': 'Писал региональный этап ВсОШ или выходил на заключительный '
                  'этап перечневой',
        'final': 'Участник финала ВсОШ или призёр перечневой',
    }
    ...
    # ⚠️ КОДЫ ПРЕЖНИЕ (22.09.2026): «Перечневые» стали «Другими перечневыми»
    # тем же кодом `listed`, данные не переписываются. Короткие подписи нужны
    # плашкам закрытого списка, выгрузке и кабинету учителя.
    OLYMPIAD_HISTORY = (
        ...
        ('hse', 'Высшая проба'),
        ('ranepa', 'РАНХиГС'),
        ('mission', 'Миссия выполнима'),
        ('spbu', 'СПбГУ'),
        ('listed', 'Другие перечневые'),
        ('none', 'Пока ни одной'),
    )
    # Раскрытый список: группы и полные подписи (ТЗ владельца 22.09.2026).
    # ⚠️ В «Миссии» тире КОРОТКОЕ (U+2013): длинное на сайте запрещено.
    OLYMPIAD_GROUPS = (
        ('Всероссийская олимпиада (ВсОШ)', (
            ('vsosh_school', 'Школьный этап'),
            ('vsosh_municipal', 'Муниципальный этап'),
            ('vsosh_region', 'Региональный этап'),
            ('vsosh_final', 'Заключительный этап'),
        )),
        ('Перечневые олимпиады', (
            ('mosh', 'МОШ'),
            ('hse', 'Высшая проба'),
            ('ranepa', 'Олимпиада РАНХиГС'),
            ('mission', 'Миссия выполнима. Твоё призвание – финансист!'),
            ('spbu', 'Олимпиада СПбГУ'),
            ('listed', 'Другие перечневые'),
        )),
    )
    OLYMPIAD_NONE = ('none', 'Пока ни одной')
    ...
    # ⚠️ С 22.09.2026 ТЕЛЕФОН НЕ СОБИРАЕМ: поля нет ни в форме, ни на странице
    # (решение владельца). Колонка и уже собранные номера остаются — стирать их
    # отдельный шаг, и он пока не сделан. Заводить поле обратно в форму нельзя.
    ...
    # Ник без «@»; нормализует форма (`clean_telegram`). P0: в модель ИИ не
    # уходит, как все поля профиля — сторож `problems/tests/profile_markers.py`.
    telegram = models.CharField('Telegram', max_length=32, blank=True, default='')
...
def suggested_points(is_test, difficulty):
    """Что подставить в поле балла на шаге «Состав» (ревью 17.08, фаза 10).

    ⚠️ ЭТО ПОДСКАЗКА КОНСТРУКТОРА, А НЕ ЗНАЧЕНИЕ ПО УМОЛЧАНИЮ В БАЗЕ.
    `default_points` остаётся прежней: по ней уже собраны существующие
    работы, и трогать её значило бы переоценить их задним числом. Здесь
    другое правило и другой смысл — «сколько эта задача, скорее всего,
    стоит»: у задачи столько баллов, какова её сложность (1–5), у теста
    один. Репетитор меняет число прямо в строке, и его выбор всегда
    главнее подсказки.

    Сложность в банке проставлена не у всех задач; ноль баллов за задачу —
    это не подсказка, поэтому у безсложностной берём середину шкалы.
    """
    if is_test:
        return Decimal('1')
    level = int(difficulty or 0)
    if not 1 <= level <= 5:
        level = 3
...
def difficulty_label(value):
    """«4,3 из 10» или None. ЕДИНСТВЕННАЯ точка форматирования.

    ⚠️ ЗАЧЕМ ФУНКЦИЯ, А НЕ ФИЛЬТР В ШАБЛОНЕ. На обзоре занятия стояло
    `{{ solo_difficulty|stringformat:"s"|cut:"." }}` — попытка убрать точку
    из «7.0». Точку она убирала вместе со смыслом: на экране выходило
    «70» по шкале от 1 до 10. Найдено, как только у индивидуального ученика
    появилась хоть одна оценка (обзор 13.08, п. 29).

    Запятая русская: `floatformat` даёт её только при русской локали, и
    полагаться на локаль ради одного числа не стоит. Нет оценок — None,
    а не ноль: ноль по шкале 1–10 означал бы оценку.
    """
    if value is None:
        return None
