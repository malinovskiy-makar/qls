...
⚠️ НА ПРОДЕ PDF НЕТ. Прод без Docker не имеет TeX Live — доступен только
`.tex` плюс Overleaf (верно было для Render, актуально пересмотреть на новом
хостинге — docs/MIGRATION-CHECKLIST.md). Это ограничение хостинга, а не кода:
локально сборка работает, на проде функция честно деградирует до `.tex` с
объяснением, а не падает ошибкой.
...
def pdf_button_enabled():
    ...
    прежнем бесплатном хостинге (Render, отключён 2026-08) всему приложению
    было отведено 512 МБ, а headless Chromium на ОДИН документ берёт
    200–400 МБ. Два учителя, нажавшие кнопку одновременно, кладут сайт при
    таком лимите памяти. Пока флаг снят, кнопки нет вовсе, а листок берётся
    через «Версия для печати» — она ничего не стоит и работает у всех.
