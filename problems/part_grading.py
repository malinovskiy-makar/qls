...
def is_machine_comment(text):
    """Это стандартная запись МАШИНЫ, а не слова преподавателя?

    ⚠️ ЗАЧЕМ. Машина пишет вердикт в то же поле, что и человек. Когда
    преподаватель ставит свой балл, её «Верно ✓» остаётся в поле и уезжает
    обратно в базу вместе с новой оценкой: на экране рядом стоят «ЧАСТИЧНО»
    и зелёное «Верно ✓». Слова машины — это её вердикт, а не комментарий,
    который надо править; человеческая оценка их отменяет.

    Список закрытый: всё, что пишут `student.views._write_auto_feedback`,
    `part_grading.auto_comment` и автоноль. Незнакомый текст считаем
    человеческим — потерять слова преподавателя хуже, чем оставить лишнее.
    """
    text = (text or '').strip()
    if not text:
        return False
    if text == BLANK_COMMENT:
        return True
    if text.startswith('Верно \u2713') or text.startswith('Неверно'):
        return True
    # Разбор по пунктам: «а) верно; б) неверно, правильный ответ: 5».
    return bool(re.match(r'^[\wа-яё]+\)\s+(верно|неверно)\b', text, re.I))
