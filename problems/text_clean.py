...
def shorten(text, limit):
    """Обрезка ПО ГРАНИЦЕ СЛОВА с многоточием. ОДНО правило на проект.

    ⚠️ Обрубок посреди слова понять нельзя: в подборе задач показывалось
    «...Всего в шта-», в шапке тепловой матрицы — «Вмешательство госу…».
    Хвостовая пунктуация снимается до многоточия: «торговля,…» читается как
    опечатка, а дефис на срезе — это остаток переноса, а не часть слова.

    ⚠️ Многоточие ставится ТОЛЬКО когда что-то отрезано. Иначе короткое
    название получало бы обещание продолжения, которого нет.
    """
    source = re.sub(r'\s+', ' ', (text or '')).strip()
    if len(source) <= limit:
        return source
    cut = source[:limit]
    # Срез пришёлся ровно на пробел — слово уже целое, резать больше нечего.
    if source[limit:limit + 1] != ' ':
        space = cut.rfind(' ')
        # ⚠️ ГРАНИЦА СЛОВА БЕЗ ОГОВОРОК. Прежнее «только если отрезаем не
        # больше половины» на коротком лимите (шапка матрицы) не срабатывало
        # вовсе, и подпись возвращалась обрубком «Спрос и предложе…».
        # Пробела внутри лимита нет только у одного длинного слова — там
        # резать по буквам и правда больше нечем.
        if space > 0:
            cut = cut[:space]
    return cut.rstrip(' ,;:.-–—') + '…'
