...
class OlympiadRef(models.Model):
    ...
    match_method = models.CharField(
        ...
        choices=[
            ...
            # ⚠️ Два текстовых метода НЕ сопоставляют со внешним индексом —
            # они разносят уже найденную привязку на текстовые копии задачи
            # внутри банка (команда find_olympiad_text_duplicates). Метаданные
            # олимпиады копируются из якорной строки буквально, поэтому
            # source_site и record_id у такой строки указывают на источник
            # ЯКОРЯ, а не на источник самой задачи-копии.
