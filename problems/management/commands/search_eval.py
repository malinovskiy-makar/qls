...
def построить_индекс(scope, из_файла=None):
    ...
    ⚠️ `из_файла` — векторы берутся из привезённого файла, а не из базы, и
    БАНК ПРИ ЭТОМ НЕ ТРОГАЕТСЯ ВОВСЕ. Это и есть то, ради чего вся
    конструкция: семнадцать вариантов формулы меряются по файлам, а в банк
    въезжает ровно один победитель.

    ⚠️ Состав индекса всё равно определяет `index_queryset` — своего фильтра
    здесь нет намеренно. Разъехавшись с прод-индексом, измеритель мерил бы
    не то, что показывает сайт. Задачи из индекса, которых нет в файле,
    считаются «нет в индексе», и их доля выводится отдельной строкой.
    """
    в_индексе = list(index_queryset(scope).values_list('id', flat=True))
    if из_файла is not None:
        матрица, место_в_файле = из_файла
        ids, строки = [], []
        for pid in в_индексе:
            i = место_в_файле.get(pid)
            if i is None:
                continue      # нет в файле — как повреждённый вектор в базе
            ids.append(pid)
            строки.append(матрица[i])
        if not строки:
            return np.empty((0, EMBEDDING_DIM), dtype=np.float32), []
        return _нормализовать(np.stack(строки)), ids
...
def загрузить_файл_векторов(prefix):
    """`(матрица, {id: строка})` из пары `<prefix>.f32` / `<prefix>.meta.json`."""
    from problems.management.commands.embeddings_check_build import load_vectors

    матрица, meta = load_vectors(prefix)
    return матрица, {pid: i for i, pid in enumerate(meta['ids'])}, meta
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help='Префикс привезённого файла векторов КОРПУСА (<...>.f32 и '
                 '<...>.meta.json). Векторы берутся оттуда, а не из базы, и '
                 'банк не трогается вовсе — так меряются семнадцать вариантов '
                 'формулы до того, как хоть один въедет в банк.')
        parser.add_argument(
            '--query-vectors', default=None,
            help='Префикс привезённого файла векторов ЗАПРОСОВ. ⚠️ Корпус и '
                 'запрос обязаны быть посчитаны ОДНИМ билдом модели: '
                 'смешение даёт необъяснимый сдвиг косинуса, который не '
                 'покраснеет ни в одной проверке. Порядок строк в файле — '
                 'порядок случаев набора.')
    ...
    def handle(self, *args, **options):
        ...
        # ── векторы корпуса и запросов из привезённых файлов ──────────
        файл_корпуса = мета_корпуса = None
        if options['vectors']:
            матрица, место_в_файле, мета_корпуса = загрузить_файл_векторов(
                options['vectors'])
            файл_корпуса = (матрица, место_в_файле)
            self.stdout.write(
                'Векторы корпуса из файла: %s (спецификация «%s», версия %s, '
                'строк %d). Банк не трогается.'
                % (options['vectors'], мета_корпуса.get('spec'),
                   мета_корпуса.get('version'), мета_корпуса.get('rows')))

        векторы_запросов = None
        if options['query_vectors']:
            if режим != 'text':
                raise CommandError(
                    '--query-vectors бессмыслен для набора в режиме %r: '
                    'запросом там служит уже посчитанный вектор задачи.' % режим)
            q_матрица, _место, мета_запросов = загрузить_файл_векторов(
                options['query_vectors'])
            if мета_запросов.get('rows') < len(случаи):
                raise CommandError(
                    'В файле запросов %d строк, а в наборе %d случаев. '
                    'Порядок строк — это контракт: недостача означает, что '
                    'файл от другого набора.'
                    % (мета_запросов.get('rows'), len(случаи)))
            # ⚠️ Смешение билдов ловится здесь, а не потом по «поиск стал
            # хуже»: если корпус и запрос посчитаны разными прогонами, у них
            # разойдутся версии библиотек, и косинус поедет молча.
            if мета_корпуса and (мета_запросов.get('versions')
                                 != мета_корпуса.get('versions')):
                raise CommandError(
                    'Версии библиотек у корпуса и запросов разные. '
                    'Корпус и запрос обязаны кодироваться ОДНИМ билдом.\n'
                    '   корпус:  %s\n   запросы: %s'
                    % (мета_корпуса.get('versions'),
                       мета_запросов.get('versions')))
            векторы_запросов = q_матрица

        # Модель нужна только текстовым наборам, и только когда запросы НЕ
        # привезены файлом. В прогоне тестов подменяется `_кодировщик_модели`
        # целиком — грузить 2,12 ГБ ради проверки склейки значит добавить
        # минуты к каждому прогону CI.
        кодировать = None
        if режим == 'text' and векторы_запросов is None:
        ...
        отчёт = {
            ...
            'vectors_file': options['vectors'],
            'query_vectors_file': options['query_vectors'],
            'formula_spec': (мета_корпуса or {}).get('spec'),
            'formula_version': (мета_корпуса or {}).get('version'),
            'slices': {},
        }

        for срез in срезы:
            отчёт['slices'][срез] = self._замер(случаи, режим, срез, ks,
                                                кодировать, файл_корпуса,
                                                векторы_запросов)
    ...
    def _замер(self, случаи, режим, срез, ks, кодировать, файл_корпуса=None,
               векторы_запросов=None):
        matrix, ids = построить_индекс(срез, файл_корпуса)
        ...
        for начало in range(0, len(случаи), БАТЧ):
            ...
            q, исключить = self._векторы_запросов(
                пачка, режим, кодировать, место,
                None if векторы_запросов is None
                else векторы_запросов[начало:начало + len(пачка)])
            if matrix.shape[0] == 0:
    ...
    def _векторы_запросов(self, пачка, режим, кодировать, место,
        ...
        if готовые is not None:
            return _нормализовать(np.asarray(готовые, dtype=np.float32)),                 [None] * len(пачка)
        if режим == 'problem':
