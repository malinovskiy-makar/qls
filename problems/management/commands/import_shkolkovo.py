...
from problems.corpus_converter.ingest import (
    ...
    imported_external_ids, require_dir,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._report(problems_dir, paths, created, skipped_existing,
                     skipped_empty, with_parts, with_answer, with_solution,
                     with_rubric, with_images, warnings, before, after,
                     do_apply, limit)

    def _report(self, problems_dir, paths, created, skipped_existing,
                skipped_empty, with_parts, with_answer, with_solution,
                with_rubric, with_images, warnings, before, after,
                do_apply, limit):
        ...
        out_path = os.path.join(out_dir, 'shkolkovo_warnings.txt')
        with open(out_path, 'w', encoding='utf-8') as f:
            f.write('\n'.join(warnings))
        lines.append(f'  предупреждения выписаны в {out_path}')
