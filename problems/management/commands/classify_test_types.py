...
def combine(source_verdict, content_verdict):
    ...
    if source_verdict in FOUR_TYPES:
        ...
        # Единственное исключение, решение владельца 2026-09-02: источник
        # объявил мультивыбор, а верным оказался ровно один вариант. Формат
        # задаёт источник, и один верный вариант в мультивыборе законен.
        # Обратное направление исключением НЕ является: если источник назвал
        # одиночный выбор, а ответов оказалось несколько, это противоречие.
        if source_verdict == T_ALL and content_verdict == T_ONE:
            return T_ALL, 'мультивыбор источника с одним верным вариантом'
