"""
Management command: import_archive3

Импортирует задачи из LaTeX ZIP-архивов Archive 3 (ОШ/ЛШ Олмат и др.).
Распаковывает все ZIP из указанной папки во временную /tmp/archive3_tex/,
...
Поддерживаемые форматы (определяется per-problem автоматически):
  Новый МатЭк: \\problem{метка}[Заголовок]{условие}{\\n подп.} + \\solution{}{решение}
  Старый:      \\problem текст условия \\Subitem вар1 \\Subitem вар2

Запуск:
    python manage.py import_archive3
    python manage.py import_archive3 --zip-dir /path/to/zips
    python manage.py import_archive3 --dry-run   # только считает, не пишет
"""
...
# ⚠️ Путь предсказуемый, и это осознанно: сюда распаковывается архив,
# и распаковка переиспользуется между запусками — иначе каждый прогон
# заново разбирал бы десятки проектов Overleaf. Команда разовая, идёт
# только на машине владельца (на сервере shell недоступен вовсе), так
# что подмена каталога другим пользователем машины в модель угроз не
# входит. Постоянного каталога здесь быть не должно — это черновик.
EXTRACT_DIR = Path('/tmp/archive3_tex')  # nosec B108
BATCH_SIZE = 50
...
class Command(BaseCommand):
    help = 'Импортирует задачи из LaTeX ZIP-архивов Archive 3 (ОШ/ЛШ Олмат и др.)'
    ...
    def handle(self, *args, **options):
        ...
        source, created_src = Source.objects.get_or_create(
            name='Overleaf Archive 3 (ОШ/ЛШ Олмат)',
            defaults={
                'kind': 'авторские материалы',
                'note': 'LaTeX файлы из Overleaf-архива 3 (ВШ/ЗШ/ЛШ/ОШ Олмат, интенсивы и др.).',
            },
        )
        self.stdout.write(
        ...
        # ── Подсчёт уникальных (для dry-run и информации) ─────────────────────
        unique_count = sum(
        ...
        self.stdout.write(
            ...
            f'уникальных (не в базе): {unique_count}'
        )
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(
                f'\nDry-run завершён.\n'
                f'  Уникальных задач: {unique_count} из {total} найденных\n'
                f'  (дублей с базой: {total - unique_count})\n'
                f'  Job #{job.pk} (не сохранён — dry-run)'
            ))
