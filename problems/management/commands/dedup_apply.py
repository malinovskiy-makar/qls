...
class Command(BaseCommand):
    ...
    # ── запись ─────────────────────────────────────────────────────────
    def _write_marks(self, решения, отпечаток_до):
        """Перезаписывает `DupMark` целиком — поэтому повторный `--apply`
        даёт то же состояние.

        Проверка защищённых полей стоит ВНУТРИ транзакции намеренно: если
        запись каким-то образом задела `Problem`, ошибка откатывает всю
        транзакцию, а не докладывает о случившемся постфактум.

        ⚠️ Граница этой защиты. Откатывается то, что сделано ВНУТРИ этой
        транзакции. Правку, закреплённую раньше, сторож заметит и назовёт
        (вторая проверка в конце `handle`), но отменить не сможет. Поэтому
        главная защита — не сторож, а отсутствие у команды хоть одного пути
        записи в `Problem`; сторож ловит тот путь, который кто-то добавит.
        """
        строки = []
        ...
        with transaction.atomic():
            ...
            if protected_digest() != отпечаток_до:
                raise CommandError(
                    'ЗАЩИЩЁННЫЕ ПОЛЯ ИЗМЕНИЛИСЬ во время записи пометок — '
                    'транзакция откачена, база осталась как была.')
