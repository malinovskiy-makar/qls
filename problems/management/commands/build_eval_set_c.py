...
# Markdown-ссылка целиком: [подпись](адрес). Вырезается ЦЕЛИКОМ, а не только
# адрес, иначе в запросе остаётся хвост «[]()» и мерится вместе с ним.
_MD_ССЫЛКА = re.compile(r'\[[^\]]*\]\([^)]*\)')
# Нумерация строки у владельца: «27. ». В .docx её не было, в .md есть.
_НУМЕРАЦИЯ = re.compile(r'^\s*\d+[.)]\s+')
...
def читать_строки(path: Path) -> list[str]:
    """Строки исходника. Понимает .docx (С14) и .md (С15 и дальше).

    Оба формата дают одно и то же — список строк «формулировка: ссылка», —
    поэтому разбор ниже у них общий, различается только чтение.
    """
    if path.suffix.lower() == '.docx':
        return читать_абзацы(path)
    if path.suffix.lower() in ('.md', '.markdown'):
        return path.read_text(encoding='utf-8').splitlines()
    raise ValueError(
        f'Неизвестный формат исходника: {path.suffix!r}. '
        f'Понимаю .docx и .md.')
...
def разобрать_строку(строка: str):
    ...
    ⚠️ ССЫЛКА ВЫРЕЗАЕТСЯ ДО ВЫЧИСЛЕНИЯ ФРАЗЫ, И ПОРЯДОК ЗДЕСЬ ВАЖЕН. В .md
    адрес приходит дважды — подписью и адресом markdown-ссылки, — поэтому
    сначала убирается ссылка целиком, и только потом то, что осталось,
    считается формулировкой. Уберёшь один адрес — в запрос уедет «[]()».
    """
    совпадение = ССЫЛКА.search(строка)
    pid = int(совпадение.group(1)) if совпадение else None
    фраза = _MD_ССЫЛКА.sub('', строка)
    фраза = ССЫЛКА.sub('', фраза)
    фраза = _НУМЕРАЦИЯ.sub('', фраза)
    фраза = фраза.strip().rstrip(':').strip()
