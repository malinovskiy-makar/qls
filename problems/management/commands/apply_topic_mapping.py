...
#
# ⚠️ БЫЛО 21, СТАЛО 23 (сессия 9, фаза 2). Двум темам с реальными данными не
# нашлось канонического дома, и обе — не мелочь: «Совокупный спрос и
# совокупное предложение» это крупная тема олимпиадной макроэкономики, её
# отсутствие было дырой в атласе, а «Введение в экономическую теорию» —
# вводная тема курса. Остальные посторонние темы сведены к канону таблицей
# соответствий (`problems/topic_merge.py`).
#
# Этот список — ЕДИНСТВЕННЫЙ источник правды: из него строятся фильтр и атлас
# каталога, колонки статистики, меню тем в подборе домашки и чипы Econ Rush.
# ---------------------------------------------------------------------------

INTRO  = 'Введение в экономическую теорию'
...
ADAS   = 'Совокупный спрос и совокупное предложение'
...
# Порядок = порядок отображения в каталоге (вводная → микро → макро → прочее).
# «Введение» стоит ПЕРВЫМ — с него начинается курс; AD-AS стоит сразу после
# «ВВП и национальные счета», рядом со своим макроблоком.
CANONICAL = [
    INTRO,
    KPV, SD, EL, CONS, FIRM, SK, MON, OLIG, GOV, TRADE, LABOR, INEQ,
    GDP, ADAS, INFL, FISC, MONET, GROWTH,
    FIN, ECNM, BEH, MATH,
]

# Приоритет при «ничьей» (два ключа на одной позиции): раньше в списке — важнее.
# ⚠️ Две новые темы стоят в КОНЦЕ приоритета намеренно: их ключевые слова
# («совокупный спрос», «введение») широкие, и при равном положении в названии
# уступить должны они, а не «Спрос и предложение».
PRIORITY = [
    OLIG, MON, SK, EL, CONS, KPV, TRADE, LABOR, INEQ,
    GDP, INFL, FISC, MONET, GROWTH, ECNM, BEH, FIN,
    FIRM, SD, GOV, MATH, ADAS, INTRO,
]
...
KEYWORDS = {
    ...
    GROWTH: [
        'экономический рост', 'экономического рост',
        'модель солоу', 'солоу', 'цикл', 'кризис',
        'долгосрочная макро', 'lr макро', 'sr макро',
    ],
    # ⚠️ «ad-as» и «совокупный спрос» ПЕРЕЕХАЛИ СЮДА из GROWTH (сессия 9):
    # пока своей темы не было, они падали в «Экономический рост и циклы», хотя
    # AD-AS это не рост и не цикл. Теперь у темы есть свой дом.
    ADAS: [
        'совокупн', 'ad-as', 'ad—as', 'as-ad', 'adas',
    ],
    INTRO: [
        'введение в эконом',
    ],
