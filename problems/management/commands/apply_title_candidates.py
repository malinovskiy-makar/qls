...
SAMPLE = 25
#: Правило 1 — самая новая часть критерия, владелец просил 15 примеров тех
#: задач, которых НЕ поймали правила 2 и 3.
SAMPLE_FIRSTLINE = 15
...
class Command(BaseCommand):
    ...
    def _report(self, корзины, путь):
        ...
        куски = [_HEAD, '<h1>Замена боевых заголовков — на приёмку</h1>',
                 ...
                 '<p class=note>Правило 2 — это «заголовок ДЛИННЕЕ 40 '
                 'символов И (дословно начинает условие ИЛИ обрывается '
                 'переносом на чёрточке)». Таблица показывает, сколько задач '
