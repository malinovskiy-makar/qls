...
Решение владельца 07.09.2026 «Критерий замены заголовков финальный»: меняем
только обрубки и технические номера, авторские клички и осмысленные заголовки
любой длины остаются. Правило отбора — одно, и живёт оно в
...
class Command(BaseCommand):
    help = ('Переносит title_candidate в боевой title для обрубков и '
            'технических номеров. Без --apply ничего не пишет.')
    ...
    def _report(self, корзины, путь):
        ...
        куски = [_HEAD, '<h1>Замена боевых заголовков — на приёмку</h1>',
                 ...
                 self._table(корзины),
                 '<h2>Из чего сложена корзина «обрубки»</h2>',
                 '<p class=note>Три правила, задача попадает в корзину, если '
                 'выполнено хотя бы одно. Столбец «правила» показывает, '
                 'сколько задач поймано каждым сочетанием: если правило ловит '
                 'мало и только вместе с другим — оно лишнее.</p>',
