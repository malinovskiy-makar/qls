...
class Command(BaseCommand):
    ...
    def _report(self, корзины, путь):
        ...
        куски = [_HEAD, '<h1>Замена боевых заголовков — на приёмку</h1>',
                 ...
                 '<p class=note>Решает ОДНО условие — есть ли в заголовке '
                 'термин словаря. Длина показана рядом, чтобы было видно, '
                 'сколько задач раньше заменялось только за неё: клетка '
                 '«длиннее 40, термин есть» снята решением владельца '
                 '07.09.</p>',
            ...
            for термин in (True, False):
                ...
                строки.append(
                    ...
                    % ('длиннее 40' if длинный else '40 и короче',
                       ...
                       'оставляем' if термин else 'заменяем'))
        строки.append('</table>')
        return '\n'.join(строки)
...
_ОПИСАНИЯ = {
    ...
    'C': '<p class=note>Спорные <code>kept</code>, которые критерий отправил '
         'на замену: в заголовке нет ни одного термина словаря экономики. '
         'Длина роли не играет — решение владельца 07.09.</p>',
    'C-оставляем': '<p class=note><b>Самая опасная половина.</b> Критерий '
                   'решил ОСТАВИТЬ: термин в заголовке есть, какой бы длины '
                   'он ни был. Если критерий ошибается — именно здесь.</p>',
