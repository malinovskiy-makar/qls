...
class TopicLabelContrastTests(TestCase):
    ...
    # ⚠️ У ВСЕХ ЧЕТЫРЁХ МЕТОК ЧЕРНИЛА — ОТДЕЛЬНЫЙ ТОКЕН, А НЕ САМ СИГНАЛ
    # (31.08.2026, вечер). Раньше зелёная и красная метки красились самим
    # сигналом поверх подложки, СДЕЛАННОЙ ИЗ ТОГО ЖЕ СИГНАЛА. Пока карточка
    # тёмной темы была глубокой (#242019), подложка выходила заметно темнее
    # буквы и пара проходила. На новой карточке #3f3f3d подложка подошла к
    # букве вплотную: зелёная метка давала 3,81, красная 3,66. Чинится это
    # не сигналом — за ним закреплён смысл, — а чернилами, ровно как уже
    # сделано у янтаря.
    PAIRS = (
        ('mastery-master', 'green-ink', 'green-tint'),
        ('mastery-good', 'amber-ink', 'amber-tint'),
        ('mastery-weak', 'error-ink', 'error-tint'),
