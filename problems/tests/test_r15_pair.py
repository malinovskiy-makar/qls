...
def _without_styles(html):
    """Страница без блоков стилей: остаётся только разметка.

    Нужна там, где проверяется наличие текста или элемента: имена классов
    встречаются и в наборе стилей, и поиск по ним краснеет всегда.
    """
...
# ⚠️ Адрес разбора запроса убран (визуальная сессия 17.08, п. 2.1): по GET
# он больше не рисует страницу, а уводит в панель шага «Что кладём» — то
# есть на первый адрес этого же списка.
GROUP_PARAM_SCREENS = [
    '/teacher/work/',
...
class GroupAndSoloPairTests(TestCase):
    ...
    def test_solo_screen_drops_the_group_only_blocks(self):
        ...
        # наступали на это пять раз.
        #
        # ⚠️ Раньше страница резалась по ПОСЛЕДНЕМУ `</style>` и бралcя
        # хвост. Приём держался на том, что все стили стоят выше разметки;
        # 04.09.2026 внизу страницы появился партиал обратной связи со своим
        # блоком стилей, и «хвост» стал куском в пару строк — тест покраснел,
        # хотя разметка не менялась. Теперь вырезаются ВСЕ блоки стилей,
        # где бы они ни стояли: свойство то же, а от порядка не зависит.
        solo = _without_styles(self.tabs(self.solo))
        group = _without_styles(self.tabs(self.group))
