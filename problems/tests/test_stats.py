...
class QueryBudgetTests(TestCase):
    ...
    #
    # ⚠️ 32 → 33 (ревью 17.08, п. 4.7). Один запрос добавил список тем, в
    # которых чаще всего промахиваешься в игре: он группирует по теме прямо
    # в базе и от числа партий не зависит. Потолок поднимаем ровно на
    # столько, сколько стоит новый блок, — иначе он перестаёт что-либо
    # ловить.
    BUDGET = 33
...
class RoleSeparationTests(TestCase):
    ...
    # ⚠️ «заморозок:» ушло из набора (ревью 17.08, п. 4.2): счётчик убран с
    # экрана как игровая механика, которая себя не объясняет. Признаком
    # геймификации вместо него стала недельная цель — её на экранах
    # репетитора и родителя нет и не должно быть.
    GAMIFICATION_MARKUP = ('class="hero"', 'id="achv-grid"', 'всего опыта',
                           'дней подряд', 'верных за неделю')
