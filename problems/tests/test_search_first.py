...
class TopicIsNotAFilterTests(TestCase):
    ...
    def test_right_topic_still_helps_the_order(self):
        # ⚠️ ЗАПРОС ОБЯЗАН НАХОДИТЬСЯ ПОИСКОМ (починено 19.08). Раньше здесь
        # стояло «кривая возможностей» — этих слов в тестовом банке нет
        # вообще, лексический поиск возвращал пусто, и подбор доходил до
        # последнего рубежа `_any_problems`, а тот берёт задачи через
        # `order_by('?')`. Тест вытягивал три СЛУЧАЙНЫЕ задачи из восьми, где
        # половина про торговлю, и краснел примерно раз из четырнадцати —
        # это и был «плавающий тест». Бонус за тему на том пути не
        # применяется вовсе, то есть проверялась случайность, а не порядок.
        rows_ppf = [{'query': 'построение КПВ', 'label': 'кпв',
    ...
    def test_query_of_this_class_is_actually_findable(self):
        """Страховка от возврата бесполезного запроса.

        Если запрос предыдущего теста перестанет находиться поиском, тот
        снова начнёт проверять случайную выдачу и снова станет плавающим —
        молча. Здесь это падает сразу и с понятной причиной.
        """
        ids, _hits, picked = hybrid.lexical_search('построение КПВ', limit=10)
        self.assertTrue(picked, 'из запроса не выделено ни одного слова')
        self.assertTrue(ids, 'запрос ничего не находит — подбор уйдёт '
                             'на случайный добор, и тест станет плавающим')
...
class StopGateTests(TestCase):
    ...
    def test_gate_shows_found_problems(self):
        ...
        # ⚠️ ПЕРЕСЧИТАН (ревью 15.08, фаза 13): в заголовке строки теперь
        # стоит и НАЗВАНИЕ, и сам запрос — «видно, какая строка запроса
        # дала эту группу задач». Требование проверки прежнее: формулировка
        # репетитора видна без раскрытия карточки.
        self.assertIn('q-text">построение КТВ', body)
        self.assertIn('искали: «построение КТВ»', body)
