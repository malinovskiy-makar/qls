...
class CardFactsTests(WorkFlowBase):
    ...
    def test_facts_are_shown(self):
        # ⚠️ ПЕРЕСЧИТАНО (визуальная сессия 17.08, п. 1.2). Сложность и
        # наличие решения печатались серым текстом «сложность 4» и «есть
        # решение» — теми же серыми чипами, что тип и число пунктов.
        # Теперь это два СИГНАЛА со своим знаком: «★ 4» и «✓ решение».
        # Проверка та же: карточка обязана сказать и то, и другое.
        html = self.client.get('%s?q=Издержки'
                               % reverse('teacher:work_pick')).content.decode()
        self.assertIn('★ 4', html)
        self.assertIn('сложность 4 из 5', html)   # подсказка у чипа
