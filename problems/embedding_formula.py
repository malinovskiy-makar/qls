...
def _b_blurb(p, spec):
    """Четвёртый блок `v2`, сразу после понятий (`API_RUN_MASTER` §9).

    ⚠️ Без метки, в отличие от соседних блоков v2: так его задаёт §9, и так
    он выглядит в `v1` и в `v2_core`. Метка изменила бы текст уже замеренных
    вариантов, а замер сравнивает составы, а не подписи.

    Цену блока меряет абляция `v2_no_blurb` — она его СНИМАЕТ."""
...
#: Девятнадцать блоков. Боевая спецификация `v2` включает семнадцать;
#: `plot` и `skills` реализованы, но в неё не входят.
BLOCKS = {
    'topics': _b_topics,
    'tags': _b_tags,
...
# не предложение: семнадцать блоков ровно в этом порядке. `ai_blurb` вернулся
# в основную формулу 07.09 — владелец подтвердил, что из его списка блок
# выпал случайно; место четвёртое, как в `API_RUN_MASTER` §9.
# ---------------------------------------------------------------------------
...
V2_BLOCKS = ('topics', 'tags', 'concepts', 'blurb', 'queries', 'find',
             'given', 'solution', 'statement', 'parts', 'hints', 'features',
             'kind', 'olympiad', 'difficulty_note', 'difficulty', 'title')

#: Блоки упора — их же повторяет `v2_focus_repeat`.
FOCUS_BLOCKS = ('topics', 'tags', 'concepts', 'find', 'given')

#: Метаданные (блоки 12–17). `v2_meta_first` переносит их в голову.
META_BLOCKS = ('features', 'kind', 'olympiad', 'difficulty_note',
               'difficulty', 'title')

V2_BUDGETS = {
    'blurb': 400,           # прежний бюджет блока, не менялся
    'find': 400,            # p90 = 279
    'given': 500,           # p90 = 463
    'solution': 800,
    'statement': 1500,
...
SPECS = {
    ...
    # Абляция: v2 БЕЗ ai_blurb. До 07.09 вариант назывался `v2_plus_blurb`
    # и блок добавлял; теперь блок стоит в боевой формуле, и мерить надо цену
    # его СНЯТИЯ. Версия оставлена прежней (103) — номера не плодим.
    'v2_no_blurb': _v2('v2_no_blurb', 103, blocks=_without('blurb')),
    ...
    # Средняя длина: между v1 (599 символов) и v2 (2 083) не было ни одного
    # варианта, и победа v1 на замере читалась бы двусмысленно — короче или
    # проще? `v2_lean` разделяет эти два эффекта: состав v2 (канонические
    # темы и теги, чищеные понятия, «Найти» и «Дано» вперёд), но без решения,
    # подсказок, запросов, ai_blurb и хвостовых метаданных.
    'v2_lean': _v2('v2_lean', 106,
                   blocks=('title', 'topics', 'tags', 'concepts', 'find',
                           'given', 'statement', 'parts'),
                   budgets=dict(V2_BUDGETS, statement=400, parts=400)),
