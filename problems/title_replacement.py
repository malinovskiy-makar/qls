...
* **C** — спорные `kept`. Заменяем, если в заголовке нет ни одного термина из
  словаря `data/econ_terms.json`; термин есть — оставляем.

  ⚠️ **Длина в этом правиле не участвует, и это решение владельца 07.09**,
  принятое на числах: «не трогаем задачи с длинным названием, но с термином
  внутри». Первый вариант критерия отправлял на замену ещё и клетку «длиннее
  40 символов, но термин есть» — 6 840 задач вроде «Кешбэк как инструмент
  ценовой дискриминации» (43 символа), которые заменялись ТОЛЬКО за длину.
  Клетка снята, и порог 40 символов из корзины C выпал целиком: осталось
  одно условие вместо двух. Длинные заголовки при этом никуда не делись —
  обрубки первой строки ловит корзина A, и она же самая большая.
...
#: Потолок длины заголовка-кандидата (инвариант §11 прогона) и заодно потолок
#: того, что команда записывает в боевой `title`. В КОРЗИНУ C НЕ ВХОДИТ —
#: решением владельца 07.09 длина оттуда убрана, см. шапку модуля.
TITLE_LIMIT = 40
...
def bucket(problem):
    ...
    if problem.title_source == 'kept':
        return 'C-оставляем' if has_econ_term(problem.title) else 'C'
    return 'D-кандидат негоден'
