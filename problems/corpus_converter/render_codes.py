...
#: ⚠️ `\b` в конце обязателен, и это не педантизм. Без него короткая форма
#: `изобрази` совпадает внутри «ИзобразИМ эти случаи на графике» — а это
#: автор ведёт РЕШЕНИЕ, а не велит ученику рисовать, и о наличии картинки
#: не говорит ничего (живая #33576, поймана выборочной проверкой глазами).
#: Так же `построй` совпал бы в «построил», «построения».
#: Длинные формы стоят в переборе РАНЬШЕ коротких: иначе `изобрази\b`
#: отвергал бы «изобразите».
_DRAW_YOURSELF_RE = re.compile(
    ...
    r'проиллюстрируйте|проиллюстрируй|отобразите|отобрази)\b'
    r'[^.!?;]{0,120}?'
