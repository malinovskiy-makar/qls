...
#: Комментарий-помета на ПЕРВОЙ строке поля (строке заголовка).
#:
#: ⚠️ Почему только первая строка и только с этими оговорками. В середине
#: текста `%` от процента не отличить: в базе лежит уже ДЕГРАДИРОВАВШИЙ
#: текст, где импорт потерял слеш у `\%` (в исходнике `30\,\%`, в базе
#: `30\,%`). Замер по 4 легаси-источникам: кандидатов на снятие 393, и
#: среди них живые проценты, у которых снятие съело бы конец фразы —
#: `$(100i)$ %, то долг…` (#30061), `($APC$) равна (в %)?` (#34178),
#: `скидка $x$ % (где $x>0$)` (#39669), `возрастает на r % по сравнению`
#: (#41701). Съесть предложение хуже, чем показать помету.
#:
#: На строке заголовка неоднозначности нет: там `%` ставили авторы —
#: имя (`%Соня П`, `%жанна`), тема (`%верхняя огибающая`), происхождение
#: (`%ВШ-2024 роботы`), заметка (`%мб поменяю`). Оговорки «нет `$` слева»
#: и «нет цифры слева» убирают остаток риска. Замер: 192 пометы у 185
#: задач, ложных срабатываний в выборке 22 штук — ноль.
#: Цифра или знак доллара СЛЕВА от `%` — признак процента, а не пометы.
_TITLE_NOTE_GUARD_RE = re.compile(r'[\d$]')

#: Пустые разделители формулы: `$$$$`, `$$ $$`, `\[\]`. Появляются там,
#: где из формулы вынули картинку или окружение. На экране дают пустой
#: отступ (код `EMPTY-MATH` аудита).
_EMPTY_MATH_RE = re.compile(r'\$\$[ \t]*\$\$|\\\[[ \t]*\\\]')

#: Осиротевшие фигурные скобки вокруг заголовка: `{Подарочная}`. Остались
#: от `\problem{\textbf{Подарочная}}`, у которого импорт снял имя команды,
#: а скобки оставил. Снимаются только целиком обрамляющие первую строку и
#: только когда внутри нет своих скобок — иначе это чужая разметка.
_LEADING_BRACE_RE = re.compile(r'^[ \t]*\{([^{}\n]{1,200})\}[ \t]*$', re.M)


def strip_tex_comments(text):
    """Убрать TeX-комментарии: в начале строк и помету на строке заголовка."""
    text = _TEX_COMMENT_RE.sub(r'\1', text)
    return strip_title_note(text)


def strip_title_note(text):
    r"""Снять авторскую помету `% …` с ПЕРВОЙ строки поля.

    Помета снимается, только если ВЕСЬ кусок строки слева от `%` не
    содержит ни цифры, ни `$`, и сам `%` не экранирован. Проверять надо
    именно всю левую часть, а не соседний символ: `50\%` и `30\,%` —
    проценты, у которых импорт уже потерял слеш, и сосед у них `\` и `,`.
    Смотреть на всю левую часть — единственный способ их не съесть."""
    ...
    if index <= 0:
        return text
    left = head[:index]
    if head[index - 1] == '\\' or _TITLE_NOTE_GUARD_RE.search(left):
        return text
    return left.rstrip() + sep + rest


def drop_empty_math(text):
    """Выбросить пустые разделители формулы (`$$$$`) — они дают пустой отступ."""
    return _EMPTY_MATH_RE.sub('', text) if text else text


def unwrap_leading_brace(text):
    r"""`{Подарочная}` отдельной строкой → `Подарочная`.

    Скобки — остаток от `\problem{…}`, у которого снято имя команды."""
    return _LEADING_BRACE_RE.sub(r'\1', text) if text else text
...
def convert_text_field(text):
    ...
    # Пустая формула и осиротевшая скобка — тоже «мусор до разбора»: обе
    # мешают следующим шагам увидеть настоящую разметку, а пустой `$$$$`
    # ещё и попал бы под защиту математики как настоящий разделитель.
    text = drop_empty_math(text)
    text = unwrap_leading_brace(text)
