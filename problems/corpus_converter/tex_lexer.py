...
def _find_close(text, start, close):
    r"""Индекс закрывающего разделителя. `\$` пропускается — экранированный
    доллар не закрывает формулу (боевой `findClose`).

    ⚠️ Пара `\\` (перенос строки) пропускается тоже, и здесь эта ошибка
    стоила дороже, чем в остальных зеркалах. Без неё в `\\$` первый слеш
    проходил обычным символом, а ВТОРОЙ вместе с `$` читался как
    «экранированный доллар»: формула не закрывалась, оба её `$`
    объявлялись ВАЛЮТОЙ, `math_canon` экранировал их в `\$` — и вся
    формула деградировала в прозу, где `\quad`/`\text` доезжают до
    экрана сырыми (живой #59624). Замер по сырью: 36 задач SolveHub.

    Пропускаются ИМЕННО `\\` и `\$`, а не «слеш и любой следующий
    символ»: последнее съело бы сам разделитель `\)` / `\]`.

    ⚠️ ЧЕТВЁРТОЕ зеркало одной логики. Остальные три — `_find_close` в
    `problems/rendering.py`, `findClose` в `templates/_katex_dollars.html`
    и `findClose` в `katex_preflight._MEASURE_JS`. Меняешь одно — меняй
    все четыре, иначе конвертер и показ разойдутся в том, где кончается
    формула.
    """
    i, n = start, len(text)
    while i < n:
