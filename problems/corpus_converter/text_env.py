...
#: Принудительный перенос строки LaTeX (`\\`, `\\*`, `\\[2mm]`).
#:
#: ⚠️ Тот же класс, что `[htpb]`: в записи НЕТ имени команды, поэтому
#: `R-CMD` шлюза (ищет `\слово`) его не видит, KaTeX молчит — математики
#: тут нет, — и обратный слеш доезжает до экрана. Свипом по корпусу трёх
#: новых источников: 174 задачи из уже прошедших шлюз, 834 случая.
#:
#: `\\` + `\n` в LaTeX — ОДИН перенос, поэтому следующий перевод строки
#: съедается: иначе markdown увидел бы пустую строку и разорвал абзац.
#: В `render_markdown` стоит `breaks: True`, и одиночный `\n` даёт ровно
#: `<br>` — семантика LaTeX сохраняется без выдумывания разметки.
#:
#: Применяется ТОЛЬКО к тексту с замаскированной математикой и
#: окружениями: внутри `cases`/матрицы/`tabular` `\\` — разделитель
#: строк, и трогать его нельзя.
