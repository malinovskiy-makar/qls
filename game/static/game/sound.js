...
(function () {
  ...
  var api = {
    ...
    /* Обратный отсчёт дуэли в духе Марио Карт (решение владельца 15.09.2026):
       на 5…1 короткий низкий гудок, на «Поехали!» длинный высокий. Звук
       выключен — отсчёт немой: переключатель проверяет сам tone(). */
    countdown: function (n) {
      if (n > 0) {
        tone({ freq: 660, dur: 0.12, type: 'square', vol: 0.3 });
      } else {
        tone({ freq: 990, dur: 0.45, type: 'square', vol: 0.35 });
      }
    },
