...
class ConsumerSurplus(Scenario):
    ...
    def injectors(self):
        return [
            Injector(
                ...
                hint=u'равновесие — там, где спрос ПЕРЕСЕКАЕТСЯ с предложением, '
                     u'а не там, где спрос упирается в ось'),
            Injector(
                'region', 'willingness',
                u'заштрихована вся площадь под спросом (готовность платить)',
                self._area_under_demand,
                hint=u'излишек потребителя — это то, что покупатели готовы были '
                     u'заплатить МИНУС то, что заплатили; площадь под спросом '
                     u'целиком — это только первое'),
            Injector(
    ...
    def statement(self, params):
        st = self.story(params)
        return (
            u'%s Спрос на %s задан уравнением %s, предложение — %s '
            u'(Q — %s, P — %s). Ученик нашёл равновесие и заштриховал '
            u'излишек %s.' % (
                st['lead'], st['good'],
    ...
    def explain(self, params, reference, injector):
        ...
        head = (
            ...
            u'Q* = %s, P* = %s. Излишек потребителя — треугольник между '
