...
class JointPPF(Scenario):
        ...
        return [
            ...
            Injector(
                ...
                hint=u'КПВ — это граница возможного: под ней всё достижимо, '
                     u'над ней — недостижимо ни при каком разделении труда'),
            Injector(
                'value', 'as_triangle',
    ...
    def statement(self, params):
        ...
        return (
            ...
            u'%s, либо %s %s %s; Б — либо %s %s %s, либо %s %s %s. Ученик '
    ...
    def explain(self, params, reference, injector):
        ...
        head = (
            u'Как надо. Альтернативная стоимость одной единицы «%s»: у А — '
            u'%s «%s», у Б — %s «%s». Дешевле «%s» обходится хозяйству %s, '
            u'поэтому сначала весь этот товар делает оно, и излом приходится '
            u'на (%s; %s). Концы совместной КПВ — (0; %s) и (%s; 0). Площадь '
