...
STORIES = [
    ...
    {'key': 'railway',
     'lead': u'Единственная железнодорожная ветка к посёлку принадлежит одной '
             u'компании — возить грузы больше нечем.',
...
class MonopolyProfit(Scenario):
    ...
    def injectors(self):
        return [
            ...
            Injector(
                ...
                self._q_at_mc,
                hint=u'равенство цены и предельных издержек — правило '
    ...
    def statement(self, params):
        ...
        return (
            ...
            u'%s Спрос на %s задан уравнением %s (Q — %s, P — %s). Предельные '
    ...
    def explain(self, params, reference, injector):
        ...
        head = (
            ...
            u'ATC = %s + %s / %s = %s. Прибыль — прямоугольник между ценой и '
            u'ATC: (%s − %s) · %s = %s.' % (
