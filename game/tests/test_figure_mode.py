...
class ClientTests(TestCase):
    ...
    def test_mode_meta_has_the_figure_entry(self):
        ...
        # ⚠️ Иконка, а не эмодзи: после слияния с main весь набор режимов
        # рисуется картинками из `templates/_icons.html` (эмодзи запрещены).
        self.assertIn("figure:  { icon: 'chart'", self.src)
        self.assertNotIn('emoji:', self.src)
