...
def _soda(p):
    return (u'В стране Альфа обсуждают, как сократить потребление сладкой '
            ...
            u'(в тыс. бутылок, цена $P$ в руб. за бутылку). С нового года '
...
def _bags(p):
    return (u'Городские власти N борются с пластиковым мусором: пакеты '
            ...
            u'цена $P$ в руб. за пакет). Совет города ввёл экологический '
...
def _energy(p):
    return (u'Депутаты решили ограничить продажу энергетических напитков '
            ...
            u'функциями {} и {} (в тыс. банок, цена $P$ в руб. за банку). '
...
def _milk(p):
    return (u'В регионе N фермеры жалуются: молоко продаётся дешевле, чем '
            ...
            u'переработчиков {} и предложение ферм {} (в тыс. литров, цена $P$ '
...
def _trains(p):
    return (u'Пригородные электрички в области N возят дачников себе в '
            ...
            u'поездок, цена билета $P$ в руб.). Область назначила '
...
def _saplings(p):
    return (u'Город N задумал озеленение: деревьев нужно много, а питомники '
            u'выращивать саженцы не спешат, ведь слишком долго ждать выручки. '
            u'Спрос на саженцы {} и предложение питомников {} (в тыс. штук, '
            u'цена $P$ в руб. за саженец). Мэрия ввела субсидию {} руб. за '
...
class TaxSubsidyArchetype(Archetype):
    ...
    def solution(self, params, solved, asked):
        ...
        steps = [
            u'Сначала посмотрим, что было до вмешательства. $Q_d = Q_s$: ${} = {} '
            ...
            u'Тогда $P_s = {} {} {} = {}$ руб., а объём находим из спроса: '
            u'$Q_1 = {}$ {} (было {}).'.format(
        ...
        if asked.key == 'budget':
            ...
            steps.append(
                u'Чистые потери образует треугольник на сделках, которые были '
