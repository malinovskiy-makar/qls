...
def _honey(p):
    return (u'Каждую субботу в городе N работает фермерский рынок мёда. '
            ...
            u'{} банок. Покупатели ведут себя наоборот — чем дороже, тем '
            u'меньше берут: {}. К вечеру рынок нащупывает цену, при которой '
...
def _bikes(p):
    return (u'В городе N открылся сезон проката велосипедов, и несколько '
            ...
            u'{}. Пока цена не та, кто-то остаётся без велосипеда или '
            u'кто-то — с непроданным складом.').format(
...
def _tulips(p):
    return (u'Перед 8 Марта на цветочном рынке города N торгуют тюльпанами. '
            ...
            u'известен по прошлым годам: {}. Цветы — товар скоропортящийся, '
...
STORIES = [
    Story('honey', u'шт.', _honey),
    Story('bikes', u'шт.', _bikes),
    Story('tulips', u'шт.', _tulips),
    Story('mugs', u'шт.', _mugs),
    Story('coffee', u'кг', _coffee),
]
...
class EquilibriumArchetype(Archetype):
    ...
    def wrappers(self):
        ...
        return [Wrapper('story', full, _market.setup_short)]
    ...
    def solution(self, params, solved, asked):
        ...
        unit_q = STORIES[params['story']].unit_q
        steps = [
            u'Равновесие — это цена, при которой рынку не с чего двигаться: '
        ...
        if asked.key == 'q_star':
            ...
            steps.append(
                ...
                u'дают $Q^* = {}$ {} — ни дефицита, ни излишка.'.format(
                    fmt_num(q_star, latex=True), unit_q))
        return steps
