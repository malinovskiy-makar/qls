...
def _aqualine(p):
    return (u'Фирма «Аквалайн» остаётся единственным в городе N производителем '
            ...
            u'цеха обходится ещё в {} тыс. руб. в месяц. Директор максимизирует '
            u'прибыль.').format(demand_q_form(p['a'], p['b']), p['mc'], p['fc'])


def _pharma(p):
    return (u'Лаборатория «Ревиталь» остаётся единственным в стране держателем '
...
def _monotown(p):
    return (u'Кирпичный завод «Гранит» остаётся единственным производством '
            ...
            u'{} руб., а печь надо топить и обслуживать круглый сезон, и это '
...
def _airport(p):
    return (u'Кофейня «Гринвуд» выиграла конкурс и стала единственным '
            ...
            u'места в терминале стоит {} тыс. руб. в месяц.').format(
                demand_q_form(p['a'], p['b']), p['mc'], p['fc'])


def _ferry(p):
    return (u'Паромная переправа «Затон» остаётся единственным способом попасть на '
            ...
            u'одну поездку стоят {} руб., а содержание причала и парома обходится в '
...
class MonopolyArchetype(Archetype):
    ...
    def solution(self, params, solved, asked):
        ...
        steps = [
            u'Перейдём от спроса к обратному спросу, выразив цену через '
        ...
        if asked.key in ('p_m', 'profit', 'dwl'):
            steps.append(
                u'Цену берём с кривой спроса, ведь это максимум, который '
        ...
        if asked.key == 'profit':
            ...
            steps.append(
                ...
                u'выбор объёма НЕ влияли, ведь они не зависят от $Q$, и в '
        ...
        if asked.key == 'dwl':
            steps.append(
                u'Чистые потери образует треугольник между спросом и $MC$ на '
